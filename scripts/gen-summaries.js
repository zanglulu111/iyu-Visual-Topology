...
const SUMMARIES_FILE = path.join(ROOT, 'public', 'data', 'codex', 'hegel', 'summaries.json');
const INDEX_FILE = path.join(ROOT, 'data', 'codex', 'philosophy', 'hegel', 'index.json');
...
fs.mkdirSync(path.dirname(SUMMARIES_FILE), { recursive: true });
fs.mkdirSync(path.dirname(INDEX_FILE), { recursive: true });

fs.writeFileSync(SUMMARIES_FILE, JSON.stringify(summaries, null, 2), 'utf-8');
fs.writeFileSync(INDEX_FILE, JSON.stringify(indexItems, null, 2), 'utf-8');

console.log(`Generated summaries.json with ${summaries.length} entries.`);
console.log(`Generated index.json with ${indexItems.length} entries at ${INDEX_FILE}.`);
