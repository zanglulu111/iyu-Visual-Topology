...
import { Settings, Copy, RefreshCw, Check, X, Save, Trash2 } from 'lucide-react';
...
export const SimpleConfigPanel: React.FC<SimpleConfigPanelProps> = ({ onClose, driverType, lang = 'CN' }) => {
  ...
  return (
    <div
      className="w-[900px] bg-[#0c0c0c]/98 backdrop-blur-3xl rounded-sm flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-500 relative"
      style={{
        border: `1px solid ${accentColor}44`,
        boxShadow: `0 0 0 1px #1a1a1a, 0 30px 100px -20px rgba(0,0,0,0.9), 0 0 40px ${accentColor}11`
      }}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-8 py-3 border-b border-zinc-900/50 shrink-0">
        <div className="flex flex-col">
          <h2 className="text-xl font-serif text-white tracking-[0.2em] leading-none uppercase">
        ...
        <div className="flex items-center gap-6">
          <div className="flex flex-col items-end mr-2">
            <span className="text-[8px] text-zinc-500 tracking-widest uppercase mb-0.5">Status</span>
            <div className="flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>
              <span className="text-[10px] text-zinc-300 font-mono tracking-tighter uppercase">Active</span>
            </div>
          </div>
          <button
            onClick={() => {
            ...
            className="flex items-center gap-2 px-5 py-2 text-black font-bold text-[11px] tracking-widest rounded-sm transition-all uppercase hover:brightness-110 active:scale-95"
            style={{
      ...
      <div className="flex-1 flex flex-col p-6 space-y-4 overflow-hidden">
        ...
        <div className="grid grid-cols-12 gap-6 items-center border-b border-zinc-900/40 pb-4">
          ...
          <div className="col-span-6 relative flex items-center gap-2">
            <div className="relative flex-1">
              <input
                ...
                placeholder="sk-..."
                className="w-full bg-black/60 border border-zinc-800 rounded-sm px-4 py-2 text-white text-sm focus:outline-none transition-all font-mono placeholder:text-zinc-800 pr-10"
            ...
            <button
              ...
              className="p-2 text-zinc-500 hover:text-red-400 border border-zinc-800 rounded-sm transition-all hover:bg-red-400/5 group"
          ...
          <div className="col-span-3 flex justify-end">
            <button
              ...
              className="flex items-center gap-2 text-zinc-200 hover:text-white transition-all text-xs font-bold uppercase tracking-wider border border-zinc-800 hover:border-zinc-700 px-4 py-2 rounded-sm disabled:opacity-30 min-w-[140px] justify-center"
            >
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="grid grid-cols-2 gap-2 overflow-y-auto no-scrollbar pr-1">
            {ENGINE_CONFIGS.map((engine) => (
              <div key={engine.id} className="bg-zinc-900/10 border border-zinc-800/40 rounded-sm p-3.5 hover:bg-white/[0.02] transition-colors group">
      ...
      {/* Footer */}
      <div className="px-8 py-2.5 border-t border-zinc-900 flex justify-between items-center bg-black/60 shrink-0">
        <button
          onClick={handleReset}
          className="text-[9px] font-bold text-zinc-700 hover:text-red-500/80 transition-colors uppercase tracking-[0.2em] flex items-center gap-2"
        >
          <RefreshCw size={10} />
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-[9px] text-zinc-800 font-mono">PROTOCOL.V.3.1.2024</span>
        </div>
      </div>
    </div>
  );
