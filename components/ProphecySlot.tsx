        ...
        <div className={containerClass}>
            ...
            {tags.length > 0 ? (
                tags.map((tag, idx) => {
                    ...
                    return (
                        <div key={tag} className="flex flex-col items-center relative group/item align-top">
                            ...
                            {!isTiny && (
                                <div className={`absolute top-[calc(100%+6px)] left-1/2 -translate-x-1/2 flex items-center gap-1 z-10 opacity-0 group-hover/item:opacity-100 transition-all duration-300 ${isRetro ? 'bg-[var(--bg-panel)]/80 backdrop-blur' : 'bg-black/80 backdrop-blur'} rounded p-1 border shadow-md ${isRetro ? 'border-[var(--border-main)]/40' : 'border-zinc-800'}`}>
                                    <button onClick={(e) => { e.stopPropagation(); onRandomizeTag?.(blockId, tag); }} disabled={isTagLocked} className={`group/btn relative flex items-center justify-center p-0.5 ${isRetro ? 'bg-[var(--bg-panel)] border-[var(--border-main)]/40 text-[var(--text-muted)] hover:text-[var(--text-main)] hover:border-[var(--border-main)]' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:bg-zinc-800 hover:text-white'} border rounded transition-colors ${isTagLocked ? 'opacity-30 cursor-not-allowed' : ''}`}>
                                        <Dice5 size={10} />
                                        <span className={`absolute top-full mt-1.5 left-1/2 -translate-x-1/2 px-2 py-1 text-[10px] font-normal whitespace-nowrap opacity-0 group-hover/btn:opacity-100 transition-opacity duration-150 pointer-events-none z-[100] rounded shadow-md ${isRetro ? 'bg-[#1A1814] text-[var(--text-main)] border border-[var(--border-main)]/50' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>{lang === 'EN' ? 'Random' : '随机'}</span>
                                    </button>
                                    <button onClick={(e) => { e.stopPropagation(); onToggleTagLock?.(blockId, tag); }} className={`group/btn relative flex items-center justify-center p-0.5 ${isRetro ? 'bg-[var(--bg-panel)] border-[var(--border-main)]/40 text-[var(--text-muted)] hover:text-[var(--text-main)]' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:bg-zinc-800 hover:text-white'} border rounded transition-colors ${isTagLocked ? (isRetro ? 'border-[var(--text-accent)] text-black bg-[var(--text-accent)]/10' : 'border-rose-500/50 text-rose-500 bg-rose-900/20') : ''}`}>
                                        {isTagLocked ? <Lock size={10} /> : <Unlock size={10} />}
                                        <span className={`absolute top-full mt-1.5 left-1/2 -translate-x-1/2 px-2 py-1 text-[10px] font-normal whitespace-nowrap opacity-0 group-hover/btn:opacity-100 transition-opacity duration-150 pointer-events-none z-[100] rounded shadow-md ${isRetro ? 'bg-[#1A1814] text-[var(--text-main)] border border-[var(--border-main)]/50' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>{isTagLocked ? (lang === 'EN' ? 'Unlock' : '解锁') : (lang === 'EN' ? 'Lock' : '锁定')}</span>
                                    </button>
                                    <button onClick={(e) => handleEditClick(tag, e)} disabled={isTagLocked} className={`group/btn relative flex items-center justify-center p-0.5 ${isRetro ? 'bg-[var(--bg-panel)] border-[var(--border-main)]/40 text-[var(--text-muted)] hover:text-[var(--text-main)]' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:bg-zinc-800 hover:text-white'} border rounded transition-colors ${isTagLocked ? 'opacity-30 cursor-not-allowed' : ''}`}>
                                        <Edit2 size={10} />
                                        <span className={`absolute top-full mt-1.5 left-1/2 -translate-x-1/2 px-2 py-1 text-[10px] font-normal whitespace-nowrap opacity-0 group-hover/btn:opacity-100 transition-opacity duration-150 pointer-events-none z-[100] rounded shadow-md ${isRetro ? 'bg-[#1A1814] text-[var(--text-main)] border border-[var(--border-main)]/50' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>{lang === 'EN' ? 'Customize' : '自定义'}</span>
                                    </button>
                                    <button onClick={(e) => { e.stopPropagation(); onRemoveTag(blockId, tag); }} disabled={isTagLocked} className={`group/btn relative flex items-center justify-center p-0.5 ${isRetro ? 'bg-[var(--bg-panel)] border-[var(--border-main)]/40 text-[var(--text-muted)] hover:text-red-700' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-red-500/50 hover:bg-red-950/20 hover:text-red-400'} border rounded transition-colors ${isTagLocked ? 'opacity-30 cursor-not-allowed' : ''}`}>
                                        <Trash2 size={10} />
                                        <span className={`absolute top-full mt-1.5 left-1/2 -translate-x-1/2 px-2 py-1 text-[10px] font-normal whitespace-nowrap opacity-0 group-hover/btn:opacity-100 transition-opacity duration-150 pointer-events-none z-[100] rounded shadow-md ${isRetro ? 'bg-[#1A1814] text-[var(--text-main)] border border-[var(--border-main)]/50' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>{lang === 'EN' ? 'Clear' : '清空'}</span>
                                    </button>
                                </div>
                            )}
                ...
                <div className={`flex flex-col items-center group/item relative cursor-pointer align-top ${isBlockLocked ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    ...
                    <div className={`absolute top-[calc(100%+6px)] left-1/2 -translate-x-1/2 flex items-center gap-1 z-10 opacity-0 group-hover/item:opacity-100 transition-all duration-300 ${isRetro ? 'bg-[var(--bg-panel)]/80 backdrop-blur' : 'bg-black/80 backdrop-blur'} rounded p-1 border shadow-md ${isRetro ? 'border-[var(--border-main)]/40' : 'border-zinc-800'}`}>
                        <button onClick={(e) => { e.stopPropagation(); onRandomizeBlock(blockId); }} disabled={isBlockLocked} className={`group/btn relative flex items-center justify-center p-0.5 ${isRetro ? 'bg-[var(--bg-panel)] border-[var(--border-main)]/40 text-[var(--text-muted)] hover:text-[var(--text-main)] hover:border-[var(--border-main)]' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:bg-zinc-800 hover:text-white'} border rounded transition-colors`}>
                            <Dice5 size={10} />
                            <span className={`absolute top-full mt-1.5 left-1/2 -translate-x-1/2 px-2 py-1 text-[10px] font-normal whitespace-nowrap opacity-0 group-hover/btn:opacity-100 transition-opacity duration-150 pointer-events-none z-[100] rounded shadow-md ${isRetro ? 'bg-[#1A1814] text-[var(--text-main)] border border-[var(--border-main)]/50' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>{lang === 'EN' ? 'Random' : '随机'}</span>
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); onToggleLockBlock(blockId); }} className={`group/btn relative flex items-center justify-center p-0.5 ${isRetro ? 'bg-[var(--bg-panel)] border-[var(--border-main)]/40 text-[var(--text-muted)] hover:text-[var(--text-main)]' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:bg-zinc-800 hover:text-white'} border rounded transition-colors ${isBlockLocked ? (isRetro ? 'border-[var(--text-accent)] text-black bg-[var(--text-accent)]/10' : (isCommercial ? 'text-cyan-400 border-cyan-400' : isExperimental ? 'text-purple-400 border-purple-400' : isAesthetic ? 'text-rose-400 border-rose-400' : isTrailer ? 'text-orange-400 border-orange-400' : 'text-gold-primary border-gold-primary')) : ''}`}>
                            {isBlockLocked ? <Lock size={10} /> : <Unlock size={10} />}
                            <span className={`absolute top-full mt-1.5 left-1/2 -translate-x-1/2 px-2 py-1 text-[10px] font-normal whitespace-nowrap opacity-0 group-hover/btn:opacity-100 transition-opacity duration-150 pointer-events-none z-[100] rounded shadow-md ${isRetro ? 'bg-[#1A1814] text-[var(--text-main)] border border-[var(--border-main)]/50' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>{isBlockLocked ? (lang === 'EN' ? 'Unlock' : '解锁') : (lang === 'EN' ? 'Lock' : '锁定')}</span>
                        </button>
                        <button onClick={handleCreateClick} disabled={isBlockLocked} className={`group/btn relative flex items-center justify-center p-0.5 ${isRetro ? 'bg-[var(--bg-panel)] border-[var(--border-main)]/40 text-[var(--text-muted)] hover:text-[var(--text-main)]' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:bg-zinc-800 hover:text-white'} border rounded transition-colors ${isBlockLocked ? 'opacity-30 cursor-not-allowed' : ''}`}>
                            <Edit2 size={10} />
                            <span className={`absolute top-full mt-1.5 left-1/2 -translate-x-1/2 px-2 py-1 text-[10px] font-normal whitespace-nowrap opacity-0 group-hover/btn:opacity-100 transition-opacity duration-150 pointer-events-none z-[100] rounded shadow-md ${isRetro ? 'bg-[#1A1814] text-[var(--text-main)] border border-[var(--border-main)]/50' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>{lang === 'EN' ? 'Customize' : '自定义'}</span>
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); onClearBlock(blockId); }} disabled={isBlockLocked} className={`group/btn relative flex items-center justify-center p-0.5 ${isRetro ? 'bg-[var(--bg-panel)] border-[var(--border-main)]/40 text-[var(--text-muted)] hover:text-red-700' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-red-500/50 hover:bg-red-950/20 hover:text-red-400'} border rounded transition-colors`}>
                            <Trash2 size={10} />
                            <span className={`absolute top-full mt-1.5 left-1/2 -translate-x-1/2 px-2 py-1 text-[10px] font-normal whitespace-nowrap opacity-0 group-hover/btn:opacity-100 transition-opacity duration-150 pointer-events-none z-[100] rounded shadow-md ${isRetro ? 'bg-[#1A1814] text-[var(--text-main)] border border-[var(--border-main)]/50' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>{lang === 'EN' ? 'Clear' : '清空'}</span>
                        </button>
                    </div>
