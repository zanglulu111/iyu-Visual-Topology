...
const titleChromaticShadow =
  '0.55px 0.24px 0 rgba(212, 93, 82, 0.36), -0.65px -0.16px 0 rgba(255,255,255,0.42), -0.8px 0 0 rgba(94, 207, 220, 0.24), 0 12px 38px rgba(0, 0, 0, 0.68)';
...
const PortalNavItem: React.FC<NavItemDef> = ({ titleCn, titleEn, onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className="mist-portal-nav-item group flex w-full flex-col items-center text-center transition-colors duration-300"
  >
...
export const UniversePortal: React.FC<UniversePortalProps> = ({
  ...
  const startCardHover = (_card: PortalCardDef, index: number, _rect: DOMRect) => {
    ...
    cardGlitchTimers.current[index] = setTimeout(() => {
      setCardGlitchActive((prev) => {
        const next = [...prev];
        next[index] = false;
        return next;
    ...
    >
      <style>{`
        .mist-portal-root {
          ...
          --portal-red: #d45d52;
          --portal-red-soft: rgba(212, 93, 82, 0.62);
          --portal-red-faint: rgba(212, 93, 82, 0.2);
          --portal-cyan: #5ecfdc;
        ...
        @keyframes mistPortalSmokeDrift {
          0%, 100% { transform: translate3d(-5%, -2%, 0) scale(1.05); opacity: 0.1; }
          35% { transform: translate3d(3%, 1%, 0) scale(1.12); opacity: 0.16; }
          70% { transform: translate3d(6%, -1.5%, 0) scale(1.08); opacity: 0.12; }
        }

        @keyframes mistPortalSmokeDriftReverse {
          0%, 100% { transform: translate3d(4%, 2%, 0) scale(1.1); opacity: 0.08; }
          45% { transform: translate3d(-3%, -1%, 0) scale(1.02); opacity: 0.14; }
          78% { transform: translate3d(-5%, 1.5%, 0) scale(1.14); opacity: 0.1; }
        }
        ...
        @keyframes mistCardEcgBreath {
          0%, 100% {
            opacity: 0.72;
            stroke: rgba(255,255,255,0.72);
            filter:
              drop-shadow(1px 0.18px 0 rgba(166,64,56,0.22))
          ...
          42% {
            ...
            filter:
              drop-shadow(1.6px 0.32px 0 rgba(188,63,57,0.36))
          ...
          64% {
            ...
            filter:
              drop-shadow(1.15px 0.24px 0 rgba(166,64,56,0.28))
              drop-shadow(0 0 6px rgba(255,255,255,0.1));
          }
        }

        @keyframes mistCardEcgGlitch {
          0%, 100% {
            opacity: 0.78;
            stroke: rgba(255,255,255,0.8);
            clip-path: inset(0 38% 0 46%);
            filter:
              drop-shadow(0.8px 0.18px 0 rgba(0,229,255,0.22))
              drop-shadow(-0.8px -0.18px 0 rgba(255,43,214,0.18));
          }
          9% {
            opacity: 1;
            stroke: rgba(255,255,255,0.98);
            clip-path: inset(0 10% 0 64%);
            filter:
              drop-shadow(2.6px 0.58px 0 rgba(0,229,255,0.56))
              drop-shadow(-2.6px -0.58px 0 rgba(255,43,214,0.5))
              brightness(1.18);
          }
          18% {
            opacity: 0.76;
            stroke: rgba(255,255,255,0.78);
            clip-path: inset(0 58% 0 14%);
            filter:
              drop-shadow(-1.4px 0.3px 0 rgba(76,110,255,0.36))
              drop-shadow(1.4px -0.3px 0 rgba(255,79,216,0.34))
              brightness(0.96);
          }
          31% {
            opacity: 0.92;
            stroke: rgba(255,255,255,0.9);
            clip-path: inset(0 18% 0 28%);
            filter:
              drop-shadow(1.8px -0.4px 0 rgba(0,229,255,0.42))
              drop-shadow(-1.8px 0.4px 0 rgba(255,45,128,0.4))
              brightness(1.1);
          }
          45% {
            opacity: 0.84;
            ...
            clip-path: inset(0 0 0 0);
            filter:
              drop-shadow(-1px 0.22px 0 rgba(0,229,255,0.28))
              drop-shadow(1px -0.22px 0 rgba(255,43,214,0.24));
          }
          58% {
            opacity: 1;
            stroke: rgba(255,255,255,0.95);
            clip-path: inset(0 46% 0 22%);
            filter:
              drop-shadow(2.2px 0.5px 0 rgba(0,229,255,0.48))
              drop-shadow(-2.2px -0.5px 0 rgba(255,45,128,0.46))
              brightness(1.14);
          }
          72% {
            opacity: 0.8;
            stroke: rgba(255,255,255,0.76);
            clip-path: inset(0 22% 0 50%);
            filter:
              drop-shadow(-1.6px 0.36px 0 rgba(76,110,255,0.32))
              drop-shadow(1.6px -0.36px 0 rgba(255,79,216,0.3))
              brightness(0.95);
          }
          84% {
            opacity: 0.96;
            ...
            clip-path: inset(0 0 0 0);
            filter:
              drop-shadow(1.2px 0.22px 0 rgba(0,229,255,0.26))
              drop-shadow(-1.2px -0.22px 0 rgba(255,43,214,0.24));
          }
        }

        @keyframes mistCardTextBreath {
          0%, 100% {
          ...
          38% {
            ...
            text-shadow:
              1.8px 0.45px 0.75px rgba(188,63,57,0.48),
              -0.8px -0.18px 0.5px rgba(255,255,255,0.32),
          ...
          62% {
            ...
            text-shadow:
              1.25px 0.3px 0.55px rgba(166,64,56,0.36),
              -0.62px -0.14px 0.4px rgba(255,255,255,0.24),
              0 2px 12px rgba(0,0,0,0.9);
          }
        }

        @keyframes mistCardDescriptionBreath {
          0%, 100% {
            color: rgba(255,255,255,0.74);
            filter: brightness(0.96);
            text-shadow:
              0.75px 0.18px 0.32px rgba(166,64,56,0.2),
              -0.45px -0.12px 0.25px rgba(255,255,255,0.14),
          ...
          42% {
            ...
            text-shadow:
              1.2px 0.28px 0.42px rgba(188,63,57,0.34),
              -0.55px -0.14px 0.3px rgba(255,255,255,0.2),
        ...
        @keyframes mistPortalModuleEnter {
          0% {
            ...
            filter: blur(2.2px) brightness(0.86);
          }
          62% {
            opacity: 1;
        ...
        @keyframes mistPortalFilmSideDecay {
          0% {
            opacity: 1;
          ...
          100% {
            opacity: 0;
            filter: contrast(1.08) brightness(0.88) saturate(0.56);
          }
        }

        @keyframes mistPortalTitleEnter {
          0% {
            opacity: 0;
        ...
        .mist-portal-entering .mist-portal-title {
          animation: mistPortalTitleEnter 980ms cubic-bezier(0.16, 1, 0.3, 1) 120ms both;
        }

        .mist-portal-entering .mist-portal-title.mist-portal-title-glitch-active,
        .mist-portal-title.mist-portal-title-glitch-active {
          animation: mistPortalTitleGlitch 1.18s ease-in-out infinite;
        }
        ...
        .mist-portal-smoke-shader {
          opacity: 1;
          filter: saturate(0.42) contrast(1.32) blur(0.08px);
          mix-blend-mode: screen;
          transform: translate3d(calc(var(--portal-mouse-x) * -15px), calc(var(--portal-mouse-y) * -15px), 0);
        ...
        .mist-portal-smoke-layer {
          ...
          background:
            radial-gradient(ellipse at 24% 42%, rgba(255, 248, 238, 0.11), transparent 34%),
            radial-gradient(ellipse at 58% 48%, rgba(255, 79, 63, 0.07), transparent 43%),
            radial-gradient(ellipse at 79% 38%, rgba(255, 255, 255, 0.055), transparent 28%),
            linear-gradient(90deg, transparent 0%, rgba(255, 255, 255, 0.045) 43%, transparent 78%);
        ...
        .mist-portal-film-side::before {
          background:
            ...
            linear-gradient(to bottom, rgba(91, 225, 242, 0.035), transparent 24%, transparent 72%, rgba(166, 64, 56, 0.1));
          mix-blend-mode: screen;
          opacity: 0.28;
        }
        ...
        .mist-portal-film-side::after {
          background:
            radial-gradient(circle at 52% 17%, rgba(166,64,56,0.88) 0 1.5px, transparent 2.2px),
            radial-gradient(circle at 48% 49%, rgba(166,64,56,0.62) 0 1px, transparent 1.8px),
            radial-gradient(circle at 56% 82%, rgba(166,64,56,0.72) 0 1.4px, transparent 2px),
            linear-gradient(to bottom, transparent 0 34%, rgba(166,64,56,0.28) 34.15% 34.45%, transparent 34.7% 100%);
          opacity: 0.42;
          mix-blend-mode: screen;
        }
        ...
        .mist-portal-shell::after {
          ...
          opacity: 0.22;
          background:
            linear-gradient(106deg, transparent 0 16%, rgba(166,64,56,0.16) 16.08% 16.16%, transparent 16.28% 100%),
            linear-gradient(82deg, transparent 0 73%, rgba(166,64,56,0.12) 73.08% 73.16%, transparent 73.28% 100%),
            radial-gradient(circle at 91% 23%, rgba(166,64,56,0.16) 0 1px, transparent 2.5px),
            radial-gradient(circle at 8% 84%, rgba(166,64,56,0.14) 0 1.2px, transparent 2.6px);
          mix-blend-mode: screen;
        }
        ...
        .mist-portal-header {
          ...
          position: relative;
          z-index: 22;
          transform: translate3d(calc(var(--portal-mouse-x) * -3px), calc((var(--portal-mouse-y) * -1.8px) + var(--portal-enter-y, 0px)), 18px);
          transform-style: preserve-3d;
          transition: transform 180ms cubic-bezier(0.16, 1, 0.3, 1);
          will-change: transform;
        }

        .mist-portal-entering .mist-portal-header {
          animation: mistPortalModuleEnter 820ms cubic-bezier(0.16, 1, 0.3, 1) 40ms both;
        }

        .mist-portal-header::after {
          content: "";
          position: absolute;
          left: 0;
          right: 0;
          ...
          pointer-events: none;
          background:
            linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.22) 7%, rgba(166,64,56,0.62) 19%, rgba(255,255,255,0.14) 34%, transparent 62%),
            linear-gradient(90deg, transparent 70%, rgba(166,64,56,0.44) 83%, transparent 100%);
          opacity: 0.48;
          box-shadow: 0 0 18px rgba(166,64,56,0.16);
        }
        ...
        .mist-portal-nav-item {
          position: relative;
        }

        .mist-portal-nav-item::after {
          content: "";
          position: absolute;
          left: 50%;
          bottom: -7px;
          ...
          box-shadow: 0 0 10px rgba(166,64,56,0.4);
        }

        .mist-portal-nav-item:hover::after {
          opacity: 0.78;
        ...
        .mist-portal-mist-toggle.is-on:hover {
          border-color: rgba(166,64,56,0.78);
          color: rgba(255,255,255,0.98);
          box-shadow:
            0 0 20px rgba(255,255,255,0.24),
            0 0 30px rgba(166,64,56,0.22),
        ...
        .mist-portal-mist-toggle::after {
          ...
          opacity: 0;
          background:
            linear-gradient(90deg, transparent, rgba(255,255,255,0.18), rgba(166,64,56,0.34), transparent),
        ...
        .mist-portal-search::before {
          ...
          box-shadow: 0 0 12px rgba(166,64,56,0.5);
        }

        .mist-portal-search:hover,
        .mist-portal-search:focus-visible {
          border-color: rgba(166,64,56,0.68);
          box-shadow:
            inset 0 0 0 1px rgba(166,64,56,0.1),
            0 0 22px rgba(166,64,56,0.1);
        }

        .mist-portal-search:hover::before,
        .mist-portal-search:focus-visible::before {
          opacity: 0.95;
        ...
        .mist-portal-left-copy {
          ...
          position: relative;
          z-index: 6;
          transform: translateX(var(--portal-left-shift-x)) translate3d(calc(var(--portal-mouse-x) * -9px), calc((var(--portal-mouse-y) * -5px) + var(--portal-enter-y, 0px)), 24px) rotateY(calc(var(--portal-mouse-x) * -0.8deg));
          transition: transform 180ms cubic-bezier(0.16, 1, 0.3, 1);
        }

        .mist-portal-entering .mist-portal-left-copy {
          animation: mistPortalModuleEnter 980ms cubic-bezier(0.16, 1, 0.3, 1) 150ms both;
        }
        ...
        .mist-portal-stage {
          ...
          transform: perspective(1700px) translate3d(calc(var(--portal-mouse-x) * 14px), calc((var(--portal-mouse-y) * 9px) + var(--portal-enter-y, 0px)), 0) rotateX(calc(var(--portal-mouse-y) * -2.1deg)) rotateY(calc(var(--portal-mouse-x) * 2.8deg));
          transform-origin: center;
          transition: transform 180ms cubic-bezier(0.16, 1, 0.3, 1);
          will-change: transform;
          isolation: isolate;
          -webkit-mask-image: radial-gradient(ellipse 94% 86% at 52% 50%, #000 0%, #000 58%, rgba(0,0,0,0.82) 73%, transparent 100%);
          mask-image: radial-gradient(ellipse 94% 86% at 52% 50%, #000 0%, #000 58%, rgba(0,0,0,0.82) 73%, transparent 100%);
        }

        .mist-portal-entering .mist-portal-stage {
          animation: mistPortalModuleEnter 1040ms cubic-bezier(0.16, 1, 0.3, 1) 260ms both;
        }
        ...
        .mist-portal-stage::before {
          background:
            linear-gradient(90deg, transparent 0%, rgba(0,229,255,0.12) 45%, rgba(166,64,56,0.28) 51%, transparent 100%),
        ...
        .mist-portal-stage::after {
          ...
          background:
            radial-gradient(ellipse at 52% 45%, transparent 0%, transparent 48%, rgba(0,0,0,0.18) 74%, rgba(0,0,0,0.68) 100%),
            linear-gradient(180deg, rgba(0,0,0,0.04) 0%, transparent 36%, rgba(0,0,0,0.42) 100%),
        ...
        .mist-portal-cards {
          position: relative;
          z-index: 16;
          transform: translate3d(calc(var(--portal-mouse-x) * 5px), calc((var(--portal-mouse-y) * 3.5px) + var(--portal-enter-y, 0px)), 0);
          transform-style: preserve-3d;
          transition: transform 180ms cubic-bezier(0.16, 1, 0.3, 1);
          will-change: transform;
        }

        .mist-portal-entering .mist-portal-cards {
          animation: mistPortalModuleEnter 920ms cubic-bezier(0.16, 1, 0.3, 1) 420ms both;
        }
        ...
        .mist-card-bg {
          ...
          transition: transform 980ms cubic-bezier(0.16, 1, 0.3, 1), opacity 980ms ease, filter 980ms cubic-bezier(0.16, 1, 0.3, 1);
        }

        .mist-portal-card:hover .mist-card-bg,
        .mist-portal-card-glitch-active .mist-card-bg,
        .mist-portal-text-system-active .mist-card-bg {
          filter: grayscale(0) saturate(1.08) contrast(1.2) brightness(0.98);
        }
        ...
        .mist-portal-card-glitch-active .mist-card-corner,
        ...
        .mist-portal-card-glitch-active .mist-card-ecg-pulse,
        .mist-portal-text-system-active .mist-card-ecg-pulse {
          animation: mistCardEcgTraceReveal 1080ms cubic-bezier(0.19, 1, 0.22, 1) 120ms both, mistCardEcgBreath 1.28s ease-in-out 1.2s infinite;
        }

        .mist-portal-card:hover .mist-card-local-ecg,
        .mist-portal-card-glitch-active .mist-card-local-ecg,
        ...
        .mist-portal-card-glitch-active .mist-card-film-frame,
        ...
        .mist-card-red-splice {
          ...
          width: clamp(1.2rem, 1.45vw, 1.7rem);
          height: 1px;
          background: linear-gradient(90deg, transparent, rgba(166,64,56,0.78), transparent);
          box-shadow: 0 0 10px rgba(166,64,56,0.28);
          opacity: 0.5;
        ...
        .mist-portal-card:hover .mist-card-red-splice,
        .mist-portal-card-glitch-active .mist-card-red-splice,
        .mist-portal-text-system-active .mist-card-red-splice {
          opacity: 0.9;
          transform: scaleX(1);
        }
        ...
        .mist-portal-text-system-active .mist-card-title {
          animation: mistTextChromaticGlitch 1.08s ease-in-out infinite;
        ...
        .mist-portal-card:hover .mist-card-description p,
        .mist-portal-card-glitch-active .mist-card-description p,
        .mist-portal-text-system-active .mist-card-description p {
          animation: mistCardDescriptionBreath 1.28s ease-in-out 1.08s infinite;
        }
        ...
        .mist-portal-card-glitch-active .mist-card-film-base,
        ...
        .mist-portal-card.mist-portal-card-glitch-active .mist-card-title,
        .mist-portal-card.mist-portal-card-glitch-active .mist-card-description p {
          animation: mistTextChromaticGlitch 620ms ease-in-out both;
        }

        .mist-portal-card.mist-portal-card-glitch-active .mist-card-ecg-pulse {
          animation: mistCardEcgTraceReveal 520ms cubic-bezier(0.19, 1, 0.22, 1) both, mistEcgSignalGlitch 620ms ease-in-out both;
        }

        .mist-portal-text-system-active .mist-card-ecg-pulse {
          animation:
            mistCardEcgTraceReveal 760ms cubic-bezier(0.19, 1, 0.22, 1) both,
            mistCardEcgGlitch 1.92s steps(2, end) infinite;
          animation-delay: calc(var(--card-index) * 110ms);
        }
        ...
        .mist-portal-card-glitch-active::before,
        ...
        .mist-portal-card-glitch-active::after,
        ...
        .mist-portal-desire-reveal {
          ...
          color: white;
          opacity: var(--desire-page-opacity, 0);
          transform: translate3d(0, var(--desire-page-y, 102vh), 0);
          will-change: transform, opacity;
        }

        .mist-portal-desire-reveal::before {
          content: "";
          position: absolute;
          inset: 0;
          background:
            radial-gradient(circle at 52% 34%, rgba(255,255,255,0.04), transparent 32%),
            linear-gradient(180deg, rgba(0,0,0,0.92), rgba(0,0,0,0.98));
        }

        .mist-portal-desire-reveal-top,
        .mist-portal-desire-reveal-bottom {
          position: relative;
          z-index: 2;
          border-color: rgba(255,255,255,0.16);
          opacity: var(--desire-bars-opacity, 0);
          transform: translate3d(0, var(--desire-bars-y, 18px), 0);
        ...
        .mist-portal-desire-reveal .desire-brand {
          display: flex;
          align-items: center;
          gap: 0.52rem;
          min-width: 0;
        ...
        .mist-portal-desire-reveal .desire-brand-mark {
          position: relative;
          width: 1.58rem;
          height: 1.05rem;
          flex: 0 0 auto;
        ...
        .mist-portal-desire-reveal .desire-brand-mark::before,
        .mist-portal-desire-reveal .desire-brand-mark::after {
          content: "";
          position: absolute;
          top: 0;
          width: 1.05rem;
        ...
        .mist-portal-desire-reveal .desire-brand-mark::before { left: 0; }
        .mist-portal-desire-reveal .desire-brand-mark::after { right: 0; }

        .mist-portal-desire-reveal .desire-brand-title {
          font-family: var(--mist-serif);
        ...
        .mist-portal-desire-reveal .desire-brand-en {
          margin-left: 0.3rem;
          color: rgba(255,255,255,0.74);
        ...
        .mist-portal-desire-reveal .desire-brand-divider {
          height: 1.45rem;
          width: 1px;
          margin-left: auto;
        ...
        .mist-portal-desire-reveal .desire-search {
          display: flex;
          align-items: center;
          gap: 0.34rem;
          min-width: 0;
        ...
        .mist-portal-desire-reveal-bottom {
          ...
          padding: clamp(1rem, 1.7vh, 1.35rem) clamp(1rem, 2.1vw, 2.5rem) 0;
          border-top: 1px solid rgba(255,255,255,0.16);
          background: linear-gradient(0deg, rgba(0,0,0,0.94), rgba(0,0,0,0.72));
        }

        .mist-portal-desire-reveal-footline {
          display: grid;
        ...
        .mist-portal-desire-reveal-footline .footline-center {
          justify-self: center;
          text-align: center;
          word-spacing: 1.1rem;
        }

        .mist-portal-desire-reveal-footline .footline-right {
          justify-self: end;
          text-align: right;
        }

        .mist-portal-desire-reveal-content {
          position: relative;
          z-index: 2;
          flex: 1;
          display: grid;
        ...
        .mist-portal-desire-reveal-stage {
          display: grid;
          grid-template-columns: minmax(18rem, 38%) 1fr;
          gap: clamp(1rem, 2vw, 3rem);
          align-items: end;
          padding: clamp(1.4rem, 3vh, 3.2rem) 0 clamp(1.25rem, 2.6vh, 2.5rem);
          border-bottom: 1px solid rgba(255,255,255,0.14);
        }

        .mist-portal-desire-reveal-title {
          display: flex;
          flex-direction: column;
          gap: 0.75rem;
        }

        .mist-portal-desire-reveal-title h1 {
          font-family: var(--mist-serif);
          font-size: clamp(3rem, 6vw, 7rem);
          line-height: 0.92;
          letter-spacing: 0.08em;
          text-shadow: 0 14px 36px rgba(0,0,0,0.78);
        }

        .mist-portal-desire-reveal-title p {
          max-width: 26rem;
        ...
        .mist-portal-desire-reveal-copy {
          align-self: center;
          justify-self: end;
          display: flex;
          flex-direction: column;
          ...
          color: rgba(255,255,255,0.66);
          font-size: clamp(0.58rem, 0.78vw, 0.92rem);
          letter-spacing: 0.24em;
          text-transform: uppercase;
        }

        .mist-portal-desire-reveal-copy-row {
          display: flex;
          align-items: center;
        ...
        .mist-portal-desire-reveal-copy-row.is-muted {
          color: rgba(255,255,255,0.48);
        }

        .mist-portal-desire-reveal-rails {
          display: grid;
        ...
        .mist-portal-desire-reveal-rail {
        ...
        .mist-portal-desire-reveal-rail .rail-number {
          display: block;
          font-size: 0.62rem;
          letter-spacing: 0.24em;
        ...
        .mist-portal-desire-reveal-rail .rail-title {
          margin-top: 0.5rem;
          font-family: var(--mist-serif);
        ...
        .mist-portal-footer {
          ...
          transform: translate3d(calc(var(--portal-mouse-x) * 3px), calc((var(--portal-mouse-y) * 2px) + var(--portal-enter-y, 0px)), 14px);
          transform-style: preserve-3d;
          transition: transform 180ms cubic-bezier(0.16, 1, 0.3, 1);
          will-change: transform;
        }

        .mist-portal-entering .mist-portal-footer {
          animation: mistPortalModuleEnter 820ms cubic-bezier(0.16, 1, 0.3, 1) 560ms both;
        }

        .mist-portal-footer::before {
          content: "";
          position: absolute;
          left: 0;
          top: -1px;
          ...
          height: 1px;
          pointer-events: none;
          background: linear-gradient(90deg, rgba(166,64,56,0.78), rgba(255,255,255,0.18), transparent);
          box-shadow: 0 0 12px rgba(166,64,56,0.22);
        }

        .mist-portal-footer button:hover {
          color: rgba(205,76,66,0.95);
