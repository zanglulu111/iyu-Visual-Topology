...
export const PreviewContentModal: React.FC<PreviewContentModalProps> = ({ isOpen, onClose, content, title, themeAccent, lang, theme = "dark" }) => {
    ...
    return (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/95 backdrop-blur-xl animate-in fade-in duration-200 p-4" onClick={onClose}>
            <div className={`w-full max-w-3xl ${theme === 'retro' ? 'bg-[var(--bg-header)] border-[#8B261D]/20' : 'bg-[#0c0c0c] border-zinc-700'} border rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-[80vh]`} onClick={e => e.stopPropagation()}>
                <div className={`p-4 border-b ${theme === 'retro' ? 'border-[#8B261D]/20 bg-[var(--bg-header)]' : 'border-zinc-800 bg-[#0a0a0a]'} flex justify-between items-center`}>
                ...
                <div className={`flex-1 overflow-y-auto custom-scrollbar p-8 ${theme === 'retro' ? 'bg-[var(--bg-header)]' : 'bg-[#0c0c0c]'}`}>
                ...
                <div className={`p-4 border-t ${theme === 'retro' ? 'border-[#8B261D]/20 bg-[var(--bg-header)]' : 'border-zinc-800 bg-[#0a0a0a]'} flex justify-end`}>
