...
export const AestheticView: React.FC<AestheticViewProps> = ({ 
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-2 pb-40">
                           ...
                           {(aestheticData.techSpecs || []).map((spec, i) => (
                               <div key={i} className={`p-6 rounded-xl group transition-all border ${theme === 'retro' ? 'bg-transparent border-[var(--border-main)] hover:border-[#8B261D]/20 shadow-none' : 'bg-zinc-900/30 border-zinc-800 hover:border-rose-500/30'}`}>
                                   ...
                                   <input 
                                       ...
                                       className={`w-full bg-transparent text-lg font-mono border-none focus:outline-none p-0 ${theme === 'retro' ? 'text-black font-bold' : 'text-white'}`}
                                   />
                               </div>
                           ))}
                           <button 
             ...
             {activeTab === 'L5_TECH' && (
                  <div className="space-y-8">
                      <div className={`border rounded-xl overflow-hidden ${theme === 'retro' ? 'bg-transparent border-[var(--border-main)] shadow-none' : 'bg-[#080808] border-zinc-800 shadow-2xl'}`}>
                          <div className={`flex items-center justify-between px-6 py-4 border-b ${theme === 'retro' ? 'bg-black/[0.02] border-[var(--border-main)] shadow-none' : 'bg-zinc-900/50 border-zinc-800'}`}>
