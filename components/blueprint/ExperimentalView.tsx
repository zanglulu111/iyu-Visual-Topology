             ...
             {activeTab === 'CONCEPT' && (
                 <>
                      <div className={`${theme === 'retro' ? 'bg-transparent border-[var(--border-main)]' : 'bg-[var(--mist-active-accent)]/10 border border-[var(--mist-active-accent)]/30'} p-10 rounded-2xl relative overflow-hidden text-center group hover:border-[var(--mist-active-accent)]/50 transition-colors border`}>
                          <h3 className={`${theme === 'retro' ? 'text-[#8B261D]' : 'text-[var(--mist-active-accent)]'} font-mono font-bold text-xs uppercase tracking-[0.3em] mb-4`}>E0. CONCEPT AXIOM</h3>
                          <textarea
                              value={experimentalData.concept}
                              onChange={(e) => handleUpdate('concept', e.target.value)}
                              className={`w-full text-2xl md:text-4xl font-serif leading-relaxed bg-transparent border-none text-center focus:outline-none focus:ring-0 resize-none p-0 placeholder-[var(--mist-active-accent)]/30 ${isConceptTemplate ? 'text-zinc-500' : (theme === 'retro' ? 'text-black' : 'text-white')}`}
                              rows={3}
                      ...
                      <div className={`${theme === 'retro' ? 'bg-transparent border-[var(--border-main)]' : 'bg-zinc-900/30 border border-zinc-800'} p-8 rounded-xl border`}>
                         <div className="flex justify-between items-center mb-6">
                             <h4 className={`${theme === 'retro' ? 'text-[#8B261D]' : 'text-[var(--mist-active-accent)]'} font-bold text-xs uppercase tracking-widest flex items-center gap-2`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className={`p-8 rounded-xl transition-colors border ${theme === 'retro' ? 'bg-transparent border-[var(--border-main)] hover:border-[#8B261D]/30 shadow-none' : 'bg-zinc-900/30 border border-zinc-800 hover:border-[var(--mist-active-accent)]/30'}`}>
                          ...
                          <textarea
                             ...
                             className={`w-full h-64 bg-transparent text-base font-mono leading-relaxed border-none focus:outline-none resize-none p-0 placeholder-zinc-600 ${theme === 'retro' ? 'text-black' : 'text-white'}`}
                 ...
                 <div className="space-y-8">
                      <div className={`p-10 rounded-xl relative overflow-hidden border ${theme === 'retro' ? 'bg-transparent border-[var(--border-main)] shadow-none' : 'bg-zinc-900/30 border border-zinc-800'}`}>
                         ...
                         <textarea
                              ...
                              className={`w-full min-h-[300px] bg-transparent text-xl md:text-2xl font-serif leading-loose border-none focus:outline-none resize-none p-0 relative z-10 placeholder-zinc-600 ${theme === 'retro' ? 'text-black' : 'text-zinc-200'}`}
