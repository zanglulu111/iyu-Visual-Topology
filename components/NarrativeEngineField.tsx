        ...
        <div className="w-full h-full flex flex-col relative bg-[var(--bg-main)] overflow-hidden transition-colors duration-500">
            ...
            <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar relative z-10">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    <div className="min-h-full flex flex-col items-center justify-start p-4 md:px-8 pt-0 md:pt-2 md:pb-60 space-y-32 md:space-y-56">
                    {isCommercial ? (
                        ...
                        <div className="flex flex-col gap-6 md:gap-10 min-w-0">
                            <div className="flex wrap justify-center items-baseline gap-y-7 gap-x-4 w-full min-w-0">{renderProphecySlot({ prefixCN: "一个处于", prefixEN: "A", blockId: "engine_m0", placeholderCN: "M0. 精神拓扑", placeholderEN: "M0. Psychic Topology" })}{renderProphecySlot({ prefixCN: "的", blockId: "engine_m1", placeholderCN: "M1. 缺失主体", placeholderEN: "M1. Subject" })}{renderProphecySlot({ prefixCN: "在遭遇了", prefixEN: "encounters", blockId: "engine_m2", placeholderCN: "M2. 真实遭遇", placeholderEN: "M2. Encounter", suffixCN: "后，" })}</div>
                            <div className="flex wrap justify-center items-baseline gap-y-7 gap-x-4 w-full min-w-0">{renderProphecySlot({ prefixCN: "试图寻找", prefixEN: "seeking", blockId: "engine_m3", placeholderCN: "M3. 欲望幻想", placeholderEN: "M3. Fantasy" })}{renderProphecySlot({ prefixCN: "却遭到", prefixEN: "blocked by", blockId: "engine_m4", placeholderCN: "M4. 大他者阻断", placeholderEN: "M4. The Other", suffixCN: "的重重阻击；" })}</div>
                            <div className="flex wrap justify-center items-baseline gap-y-7 gap-x-4 w-full min-w-0">{renderProphecySlot({ prefixCN: "哪怕面临", prefixEN: "risking", blockId: "engine_m6", placeholderCN: "M6. 终极代价", placeholderEN: "M6. Stakes", suffixCN: "的巨大代价，" })}</div>
                            <div className="flex wrap justify-center items-baseline gap-y-7 gap-x-4 w-full min-w-0">{renderProphecySlot({ prefixCN: "他决定通过", prefixEN: "he uses", blockId: "engine_m5", placeholderCN: "M5. 行动驱力", placeholderEN: "M5. Drive", suffixCN: "抵抗，" })}{renderProphecySlot({ prefixCN: "最终导向", prefixEN: "leading to", blockId: "engine_m7", placeholderCN: "M7. 存在落点", placeholderEN: "M7. Resolution" })}</div>
                        </div>
                    )}
                    </div>
                </div>
