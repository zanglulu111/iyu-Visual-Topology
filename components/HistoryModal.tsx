...
export const HistoryModal: React.FC<HistoryModalProps> = ({ history, onRestore, onClear, onClose, lang = 'CN' }) => {
    ...
    const getDriverColorClass = (driverId: DriverType) => {
        if (driverId === DriverType.COMMERCIAL) return 'text-mist-cyan border-mist-cyan/50';
        return 'text-[var(--mist-archive-red)] border-[var(--mist-archive-red-soft)]';
    };

    const getStageLabel = (item: HistoryItem) => {
        if (theme === 'retro') {
            if (item.type === 'METONYMY') {
                return { cn: '换喻记忆点', en: 'MEMORY POINT', className: 'text-[#8B261D] border-[#8B261D]/30 bg-[#8B261D]/5', icon: 'MEM' };
            }
            if (item.type === 'BIBLE') {
                return { cn: '创意圣经', en: 'BIBLE', className: 'text-[#8B261D] border-[#8B261D]/30 bg-[#8B261D]/5', icon: 'BIB' };
            }
            if (item.treatments && item.treatments.length > 0) {
                return { cn: '分歧点', en: 'DIVERGENCE', className: 'text-[#8B261D] border-[#8B261D]/30 bg-[#8B261D]/5', icon: 'DIV' };
            }
            return { cn: '引擎DNA', en: 'ENGINE DNA', className: 'text-zinc-600 border-zinc-400 bg-zinc-100', icon: 'DNA' };
        }
        if (item.type === 'METONYMY') {
            return { cn: '换喻记忆点', en: 'MEMORY POINT', className: 'text-[var(--mist-archive-red)] border-[var(--mist-archive-red-soft)] bg-[var(--mist-archive-red-faint)]', icon: 'MEM' };
        }
        if (item.type === 'BIBLE') {
            return { cn: '创意圣经', en: 'BIBLE', className: 'text-[var(--mist-archive-red)] border-[var(--mist-archive-red-soft)] bg-[var(--mist-archive-red-faint)]', icon: 'BIB' };
        }
        if (item.treatments && item.treatments.length > 0) {
            return { cn: '分歧点', en: 'DIVERGENCE', className: 'text-[var(--mist-archive-red)] border-[var(--mist-archive-red-soft)] bg-[var(--mist-archive-red-faint)]', icon: 'DIV' };
        }
        return { cn: '引擎DNA', en: 'ENGINE DNA', className: 'text-zinc-500 border-zinc-700 bg-zinc-900/50', icon: 'DNA' };
    };

    const getSaveTypeLabel = (item: HistoryItem) => {
        if (item.type === 'METONYMY') {
            return { cn: '手动', en: 'MANUAL', className: theme === 'retro' ? 'text-[#8B261D] border-[#8B261D]/30 bg-[#8B261D]/5' : 'text-[var(--mist-archive-red)] border-[var(--mist-archive-red-soft)] bg-[var(--mist-archive-red-faint)]' };
        }
        return { cn: '自动', en: 'AUTO', className: theme === 'retro' ? 'text-zinc-700 border-zinc-500/30 bg-zinc-100' : 'text-zinc-300 border-white/20 bg-white/[0.04]' };
    };
            ...
            return (
                <button key={item.id} onClick={() => setSelectedItem(item)} className={`w-full text-left p-4 border-b ${theme === 'retro' ? 'border-[#8B261D]/10 hover:bg-[#EBE5D5]' : 'border-zinc-800/50 hover:bg-zinc-900/30'} transition-all group ${isSelected ? (theme === 'retro' ? 'bg-[#E5DFCD] border-l-[3px] border-l-[#8B261D] border-y-[#8B261D]/20 shadow-sm' : 'bg-zinc-900/80 border-l-2 ' + borderColor.replace('/50', '')) : 'border-l-[3px] border-l-transparent'}`}>
                    <div className="flex flex-col gap-2 w-full">
                        <h4 className={`font-serif text-sm truncate ${theme === 'retro' ? 'text-[#3E110D]' : textColor} ${isSelected ? 'font-bold' : 'opacity-80 group-hover:opacity-100'}`}>{item.blueprint?.narrative?.title || item.treatments?.[0]?.title || (lang === 'EN' ? "Untitled Draft" : "未命名草稿")}</h4>
                        <div className="flex wrap items-center gap-2">
                            <span className={`text-[8px] font-bold uppercase tracking-wider ${textColor} ${borderColor} ${theme === 'retro' ? 'bg-white/50' : 'bg-black/20'} px-1.5 py-0.5 rounded border`}>{item.type === 'METONYMY' ? 'METONYMY' : item.driverName}</span>
    ...
    return (
        <div className="mist-archive-overlay fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4 animate-in fade-in duration-300">
            <div className={`mist-history-modal mist-archive-modal w-full h-full max-w-[1600px] max-h-[90vh] ${theme === 'retro' ? 'bg-[#F9F7F1]' : 'bg-[#0c0c0c]'} border ${theme === 'retro' ? 'border-[#8B261D]' : 'border-zinc-800'} rounded-2xl shadow-2xl flex overflow-hidden relative`}>
                <div className={`w-80 border-r ${theme === 'retro' ? 'border-[#8B261D]/20 bg-[#F4EFE0]' : 'border-zinc-800 bg-[#080808]'} flex flex-col shrink-0`}>
                    <div className={`p-6 border-b ${theme === 'retro' ? 'border-[#8B261D]/20 bg-[#F4EFE0]' : 'border-zinc-800 bg-[#0a0a0a]'}`}>
                        ...
                        <div className={`flex ${theme === 'retro' ? 'bg-[#E5DFCD] border-black/10' : 'bg-zinc-900 border-zinc-800'} p-1 rounded-lg border mb-6 gap-1`}>
                            ...
                            <button 
                                onClick={() => { setSidebarTab('METONYMY'); setSelectedItem(null); }} 
                                    className={`flex-1 py-1.5 text-[8px] font-bold uppercase tracking-widest rounded transition-all flex items-center justify-center gap-1 ${sidebarTab === 'METONYMY' ? (theme === 'retro' ? 'bg-white text-[#8B261D] shadow-sm' : 'bg-[var(--mist-archive-red-faint)] text-white border border-[var(--mist-archive-red-soft)]') : (theme === 'retro' ? 'text-zinc-600 hover:text-black hover:bg-white/50' : 'text-zinc-500 hover:text-white')}`}
                            >
                                <Save size={10} />
                                {lang === 'CN' ? '换喻' : 'Metonymy'}
                            </button>
                            <button 
                                ...
                                className={`flex-1 py-1.5 text-[8px] font-bold uppercase tracking-widest rounded transition-all flex items-center justify-center gap-1 ${sidebarTab === 'COLLECTION' ? (theme === 'retro' ? 'bg-white text-[#8B261D] shadow-sm' : 'bg-[var(--mist-archive-red-faint)] text-white border border-[var(--mist-archive-red-soft)]') : (theme === 'retro' ? 'text-zinc-600 hover:text-black hover:bg-white/50' : 'text-zinc-500 hover:text-white')}`}
                            >
                                <Bookmark size={10} />
                        ...
                        <div className="flex items-center gap-3">
                            <div className={`p-2 ${theme === 'retro' ? 'bg-white text-[#8B261D]' : 'bg-zinc-800 text-[var(--mist-archive-red)]'} rounded-lg`}>{sidebarInfo.icon}</div>
                            <div>
                        ...
                        <div className="flex items-center gap-2 h-10">
                            {selectedItem && (
                                <button 
                                    ...
                                    className={`mist-app-primary-action h-full ${theme === 'retro' ? 'bg-[#8B261D] text-white hover:bg-[#6D1E16]' : 'bg-white hover:bg-zinc-200 text-black'} px-5 rounded-lg text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 shadow-sm transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed`}
                                >
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        {isFetchingDetail ? (
                            <div className="flex-1 flex flex-col items-center justify-center h-full gap-4 text-zinc-500">
                                <Wand2 size={32} className="animate-spin opacity-50 text-[var(--mist-archive-red)]" />
                                    ...
                                    <div className="animate-in fade-in slide-in-from-bottom-2">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                            {selectedItem.treatments?.map((t, idx) => {
                                                ...
                                                return (
                                                    <button key={idx} onClick={() => setSelectedTreatmentId(t.id)} className={`mist-archive-panel text-left p-6 rounded-xl border transition-all relative group flex flex-col h-full ${isSelected ? (theme === 'retro' ? 'bg-[#E5DFCD] border-[#8B261D] shadow-sm' : 'bg-zinc-900 border-[var(--mist-archive-red)] ring-1 ring-[var(--mist-archive-red-soft)]') : (theme === 'retro' ? 'bg-white border-black/10 hover:bg-[#EBE5D5] hover:border-[#8B261D]/30' : 'bg-black/40 border-zinc-800 hover:border-zinc-600')}`}>
                                                        <div className="flex items-center justify-between mb-4">
                                                            <span className={`text-[10px] font-bold px-2 py-1 rounded ${isSelected ? 'bg-[var(--mist-archive-red-faint)] text-white border border-[var(--mist-archive-red-soft)]' : 'bg-zinc-800 text-zinc-400'}`}>{t.type}</span>
                                                            {hasBible && <span className="flex items-center gap-1 text-[9px] font-bold text-zinc-200 bg-white/[0.04] px-2 py-0.5 rounded border border-white/20"><Check size={10} /> {lang === 'EN' ? "GENERATED" : "已生成"}</span>}
                                                        </div>
                                    ...
                                    <div className="animate-in fade-in slide-in-from-bottom-2 max-w-5xl mx-auto space-y-8">
                                        ...
                                        {Array.isArray(currentBlueprint?.metonymyData?.screenplay) && (currentBlueprint?.metonymyData?.screenplay as any[]).map((section: any) => (
                                            <div key={section.id} className={`${theme === 'retro' ? 'bg-[#F4EFE0] border-black/10' : 'bg-zinc-900/30 border-zinc-800'} border p-8 rounded-xl mb-6`}>
                                                <div className={`flex items-center gap-2 mb-4 ${theme === 'retro' ? 'text-[#8B261D]' : 'text-[var(--mist-archive-red)]'} font-bold uppercase tracking-widest text-xs`}><div className={`w-2 h-2 ${theme === 'retro' ? 'bg-[#8B261D]' : 'bg-[var(--mist-archive-red)]'} rounded-full`}></div>{section.title}</div>
