...
export const AestheticInputSidebar: React.FC<AestheticInputSidebarProps> = ({
  ...
  return (
    <div className={`
      ...
      fixed top-14 left-0 bottom-14 w-[380px] bg-[var(--bg-main)] border-r border-[var(--border-main)] 
      ${theme === 'retro' ? 'shadow-none' : (isOpen ? 'shadow-[20px_0_50px_rgba(0,0,0,0.5)]' : '')}
      ${isOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0 pointer-events-none'}
    `}>
      {/* Header */}
      <div className={`p-6 pb-5 flex items-center justify-between border-b ${theme === 'retro' ? 'border-[var(--border-main)]' : 'border-zinc-800 bg-zinc-900/40'} shrink-0 transition-colors duration-500`}>
        <div className="flex items-center gap-3">
          <MessageSquare size={18} className={theme === 'retro' ? 'text-[#8B261D]' : "text-rose-400"} />
          <div>
            <span className={`text-xs font-black uppercase tracking-[0.25em] block ${theme === 'retro' ? 'text-black' : 'text-zinc-100'}`}>
              {lang === 'EN' ? "SPEC PARSER" : "参数解析器"}
            </span>
            <span className={`text-[9px] font-mono uppercase tracking-wider block mt-1 ${theme === 'retro' ? 'text-black/40' : 'text-zinc-600'}`}>
              AI Reverse Mapping
            </span>
          </div>
        </div>
        <button
          onClick={onClose}
          className={`p-1.5 transition-all rounded-full ${theme === 'retro' ? 'hover:bg-black/5 text-black/40 hover:text-black' : 'hover:bg-zinc-800 text-zinc-600 hover:text-white'}`}
        >
          <X size={18} />
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col p-6 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar mb-4 space-y-4 pr-2">
          <div className={`p-4 rounded-xl border ${theme === 'retro' ? 'bg-transparent border-[var(--border-main)]' : 'bg-zinc-900/50 border-zinc-800/50'}`}>
            <p className={`text-xs leading-relaxed ${theme === 'retro' ? 'text-black/60 font-serif' : 'text-zinc-500'}`}>
              {lang === 'EN'
