...
export const ENGINE_M0_NEUROSIS_HYSTERIA: LibraryItemDef[] = [
  ...
  { 
    ...
    logic: "主体的 M0（身份）是不稳定的。叙事轨迹表现为在不同社会标签（Slot3）之间的快速滑移。",
    logicEn: "The subject's M0 (Identity) is unstable. The narrative trajectory manifests as rapid sliding between different social labels (Slot3).",
    patch: {
  ...
  { 
    ...
    logic: "符号界（Slot7）的压力直接释放到感官系统（Slot5）。主体的 M6（代价）直接表现为身体功能的暂时丧失。",
    logicEn: "Symbolic pressure (Slot7) is directly released into the sensory system (Slot5). The subject's M6 (Stakes) directly manifests as temporary loss of bodily function.",
    patch: {
  ...
  { 
    ...
    logic: "外部反馈（Slot8）被直接等同于主体的存在（M0）。叙事重点在于引发波澜而非解决问题。",
    logicEn: "External feedback (Slot8) is equated with subject existence (M0). Focus is on causing waves rather than solving problems.",
    patch: {
  ...
  { 
    ...
    logic: "拒斥逻辑（M5=Not Slot3）。主体的行动（M5）必须被设为对当前秩序（Slot3）的随机阻断。叙事中没有建设，只有拒绝。",
    logicEn: "Refusal logic (M5 = Not Slot3). Subject's actions (M5) must be set as random interruptions of the current order (Slot3). No construction, only rejection.",
    patch: {
  ...
  { 
    ...
    logic: "M0（自我镜像）彻底崩塌。叙事中出现语言的断裂、逻辑的停滞（Slot1停止），主体陷入原始的丧失状态。",
    logicEn: "M0 (Self-mirror) collapses completely. Breaks in language and stagnation in logic (Slot1 stop) occur; subject falls into primal loss state.",
    patch: {
  ...
  { 
    ...
    logic: "主体的 M0 被彻底商品化（Slot3化）。叙事中充满了各种‘为了迎合’而进行的自我折损。",
    logicEn: "Subject's M0 is completely commodified (Slot3-ized). Narrative filled with self-impairment for the sake of 'fitting in'.",
    patch: {
  ...
  { 
    ...
    logic: "负性主体的生成。通过在叙事流中制造巨大的空白（Null M0/Missing Slot8），来迫使所有的 M4 角色投入算力去填补这个黑洞。",
    logicEn: "Negative subject generation. Creating massive blanks in the narrative flow (Null M0/Missing Slot8), forcing other characters to fill the black hole.",
    patch: {
