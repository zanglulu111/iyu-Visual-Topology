...
export const ENGINE_M0_NEUROSIS_PHOBIA: LibraryItemDef[] = [
  ...
  { 
    ...
    logic: "主体的 M0（身份）必须通过一个‘中介物’（M3）来维持。缺乏该物，主体将立刻遭遇 M1（缺失）。",
    logicEn: "The subject's M0 (Identity) must be maintained through a 'mediator' (M3). Without it, the subject immediately encounters M1 (Lack).",
    patch: {
  ...
  { 
    ...
    logic: "在 SUR1（空间轴）上强制标记一个不可接触的变量。主体的 M5（路径）必须始终以该点为圆心进行逃离。",
    logicEn: "Forcibly mark an untouchable variable on SUR1 (Space Axis). M5 (Path) must revolve around avoiding this center.",
    patch: {
  ...
  { 
    ...
    logic: "将 M1（虚无）的质感置换（Displacement）到一个具体的 M3（恐惧客体）上。主体的叙事冲突（M2）高度具体化。",
    logicEn: "Displace the texture of M1 (Void) onto a specific M3 (Object). Narrative conflict (M2) is highly concretized.",
    patch: {
  ...
  { 
    ...
    logic: "在 SUR1（时空转场）之间强行插入一个校验函数。主体只有执行完 M5（仪式动作）才能获得下一个阶段的权限。",
    logicEn: "Forcibly insert an authentication function between SUR1 (Transitions). Access to next phase requires M5 (Ritual action).",
    patch: {
  ...
  { 
    ...
    logic: "主体的 M0（身份完整度）与某一具体 M3（对象a的化身）高度耦合。叙事公式：Anchor = True -> Lack = False。",
    logicEn: "Subject M0 (Integrity) is coupled with a specific M3. Formula: Anchor = True -> Lack = False.",
    patch: {
  ...
  { 
    ...
    logic: "物理环境（SUR1 / 拓扑空间）对 M0（完整度）的直接侵蚀。主体无法在非标状态的空间中维持逻辑自洽。",
    logicEn: "Enviro-SUR1 (Topological Space) directly eroding M0 (Integrity). Subject cannot maintain consistency in non-standard spaces.",
    patch: {
  ...
  { 
    ...
    logic: "物理/象征空间（SUR1/M4）防御加固逻辑。主体的 M5（行动）大部分用于检查和加强物理/象征性的围墙。",
    logicEn: "Spatial (SUR1/M4) defense logic. M5 (Action) is mostly spent checking and reinforcing physical/symbolic walls.",
    patch: {
