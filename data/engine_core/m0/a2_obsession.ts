...
export const ENGINE_M0_NEUROSIS_OBSESSION: LibraryItemDef[] = [
  ...
  { 
    ...
    logic: "世界的物理/社会规则（Slot3）必须被绝对化。一旦 M2 破坏了规则，主体将丧失基本的 M5 行动能力。",
    logicEn: "Worldly physical/social rules (Slot3) must be absolutized. Once M2 breaks a rule, the subject loses basic M5 action capacity.",
    patch: {
      mechanics: "基础神经症协议 + [规则刚性 = 1.0; 动力对规则敏感度 = 最大]",
      mechanicsEn: "Base_NEUROSIS + [Slot3_Rigidity = 1.0; M5_Sensitivity_to_Slot3 = Max]",
  ...
  { 
    ...
    logic: "时空轴（Slot1）的不稳定性。每发生一次关键事件，主体必须在事后用内耗式逻辑将其重构为‘符合预期’。",
    logicEn: "Instability of the timeline (Slot1). After every key event, the subject must post-hoc reconstruct it as 'expected' through exhausting logic.",
    patch: {
  ...
  { 
    ...
    logic: "叙事视角（Slot10）的强制游离。必须使用第三人称视角审视第一人称的遭遇，确保主体不被创伤吞没。",
    logicEn: "Forced dissociation of perspective (Slot10). Must use a third-person gaze to examine first-person encounters, ensuring the subject isn't swallowed by trauma.",
    patch: {
      mechanics: "基础神经症协议 + [游离指数 = 1.0; 视角覆盖 = 外部]",
      mechanicsEn: "Base_NEUROSIS + [Dissociation_Index = 1.0; Slot10_Override = External]",
