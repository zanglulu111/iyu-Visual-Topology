import { LibraryItemDef } from '../../../types';

export const OUTCOMES_GROUP_B: LibraryItemDef[] = [
    {
        id: "res_becoming_monster",
        name: "主权者的内投", nameEn: "Introjection of the Master",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "主体为了战胜恶龙，最终完全内化了恶龙的代码法则。手段吞噬了目的，主体成为自己曾经最憎恨的压迫者。",
        defEn: "To defeat the dragon, the subject ultimately fully internalizes the dragon's code and laws. The means devour the ends; the subject becomes the oppressor they once most despised.",
        core: "尼采凝视深渊的具象化。在结构意义上，大他者从未被推翻，它只是换了一具更年轻锋利的肉身宿主。 | Outcome: 宿主的接力。",
        coreEn: "The materialization of Nietzsche staring into the abyss. Structurally, the Big Other was never overthrown; it merely acquired a younger, sharper fleshly host. | Outcome: Relay of the host.",
        patch: {
            mechanics: "使用敌人的核心武器或机制实现了反杀，但在结算动画中，主角的行为模式、权限甚至外貌也完全覆盖为敌人的特征。",
            mechanicsEn: "Uses the enemy's core weapon or mechanics to achieve the counter-kill, but in the resolution, the protagonist's behavior, permissions, or even appearance are completely overwritten by the enemy's traits.",
            aesthetic: "主角穿上了死神的黑色制服，熟练地发出了同样的命令。光轴完全对称，强调权力的冰冷传承。",
            aestheticEn: "The protagonist dons the black uniform of the grim reaper, skillfully issuing the identical command. Perfectly symmetrical lighting, emphasizing the frigid succession of power.",
            runtime: "若主角的【暴戾值】高于【理念值】，系统将平滑激活【大他者权限转移（Master Transfer）】协议。",
            runtimeEn: "If the protagonist's [Violence] exceeds [Ideology], the system smoothly activates the [Master Transfer] protocol."
        }
    },
    {
        id: "res_compromise",
        name: "自愿的阉割", nameEn: "Symbolic Castration",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "在承受了足够的创伤后，主体主动缴械，切断了最后一点“对象a”的渴望，乖乖躺入结构预留的模具中成为螺丝钉。",
        defEn: "After enduring enough trauma, the subject voluntarily disarms, severing the final shred of yearning for 'object a', and obediently crawls into the mold reserved by the structure.",
        core: "比死亡更悲哀的拓扑平息。灵魂被抽离了棱角，肉体以功能性的方式苟活。你没死，只是你不再是“你”了。 | Outcome: 主体性的化学稀释。",
        coreEn: "A topological flattening sadder than death. The soul's sharp edges are drained; the body survives merely functionally. You aren't dead, you just are no longer 'you'. | Outcome: Chemical dilution of subjectivity.",
        patch: {
            mechanics: "放弃了所有的隐藏支线与非凡技能，回归主线的最低保底形态，打卡上班。",
            mechanicsEn: "Abandons all hidden sub-quests and extraordinary skills, reverting to the lowest baseline form of the main quest, punching the clock for work.",
            aesthetic: "色温下降至极其平庸的日光灯白。主角换上统一的制服，混入下班人潮，动作机械。没有配乐，只有沉闷的脚步声。",
            aestheticEn: "Color temperature drops to a profoundly mediocre fluorescent white. The protagonist dons a uniform, blending into the commuter crowd with mechanical motions. No score, only dull footsteps.",
            runtime: "检测到【行动抗印（Drive Resistance）】清零时触发，将角色的控制权部分收归系统AI。",
            runtimeEn: "Triggered when [Drive Resistance] zeros out, partially reverting character control to the system AI."
        }
    },
    {
        id: "res_blue_pill",
        name: "拜物教式的否认", nameEn: "Fetishistic Disavowal",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "“我很清楚这全是谎言，但我依然选择相信。” 主体为了维持快感生态，刻意吞下蓝药丸，拒绝直面实在界。",
        defEn: "'I know very well this is all a lie, but nevertheless I choose to believe it.' To maintain the hedonic ecology, the subject deliberately swallows the blue pill to refuse facing the Real.",
        core: "《黑客帝国》里的塞弗。真相既寒冷又带刺，于是退回温暖的羊水幻象中。理智上的清醒与行动上的沉沦并存。 | Outcome: 认知与躯体的分裂。",
        coreEn: "Cypher in The Matrix. Truth is cold and thorny, hence retreating to the warm amniotic illusion. Coexistence of intellectual lucidity and physical degradation. | Outcome: Split between cognition and soma.",
        patch: {
            mechanics: "在破解迷局的最后一环，主角调转枪头射碎了真相的发生器，然后将VR眼镜（或幻觉剂）深深推入自己的神经。",
            mechanicsEn: "At the final link to solve the puzzle, the protagonist turns the gun to shatter the truth generator, then plunges the VR headset (or hallucinogen) deep into their own nerves.",
            aesthetic: "色彩饱和度被人为拉至失真的霓虹极值。笑容伴随着眼角的泪水，呈现出一种病态的甜美感。",
            aestheticEn: "Color saturation is artificially cranked to distorted neon extremes. A smile accompanied by a tear at the corner of the eye, conveying a pathological sweetness.",
            runtime: "执行此落点后，视界的【真实滤镜】永久解除，系统开始无限制推送廉价的【虚假奖励多巴胺】。",
            runtimeEn: "After executing this resolution, the [Real Filter] of the FOV is permanently disabled, and the system begins limitlessly pushing cheap [False Reward Dopamine]."
        }
    },
    {
        id: "res_new_tyrant",
        name: "父性隐喻的继承", nameEn: "Succession of the Name-of-the-Father",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "斩杀了旧王不仅没有带来自由，反而让主体成为了新的“大他者律法”。反叛在登顶的一刻闭合成了律法的新一环。",
        defEn: "Slaying the old king brought no freedom; instead, the subject became the new 'Law of the Big Other'. Rebellion closes its loop into a new link of the Law the moment it breaches the summit.",
        core: "弑父就是为了成为父亲。当你坐上那把象征绝对权力的椅子，这把椅子瞬间篡改了你对宇宙的解释权。 | Outcome: 个体的图腾化。",
        coreEn: "Patricide precisely to become the father. When you sit in the chair symbolizing absolute power, the chair instantly hijacks your right to interpret the universe. | Outcome: Totemization of the individual.",
        patch: {
            mechanics: "旧的统治机制被原封不动地保留并打上了主角的名字标识，原本发誓要打破的剥削系统现在由主角亲自监管运行。",
            mechanicsEn: "The old ruling mechanics are kept entirely intact but labeled with the protagonist's name; the exploitative system they swore to break is now overseen by the protagonist themselves.",
            aesthetic: "冷酷的低照度室内。主角在黑暗中坐在王座上，原本热血的面部逐渐被权力的阴影掩盖。镜头隐没在深渊感中。",
            aestheticEn: "Cold, low-illumination interior. The protagonist sits on the throne in the dark; their once fiery face gradually obscured by the shadow of power. The camera vanishes into an abyssal sensation.",
            runtime: "触发条件：主体的【规则掌控力（Rule Mastery）】越过安全阈值，直接触发系统的【父位继承（Paternal Succession）】方法。",
            runtimeEn: "Trigger condition: The subject's [Rule Mastery] crosses the safe threshold, directly triggering the system's [Paternal Succession] method."
        }
    },
    {
        id: "res_replaced",
        name: "主体的可替换性", nameEn: "Fungibility of the Subject",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "主角死了或离场，但他的工作、地位甚至妻子立刻被一个与他具有相同参数的“替代能指”所填补。",
        defEn: "The protagonist dies or exits, but their job, status, or even spouse is immediately filled by a 'substitute signifier' possessing identical parameters.",
        core: "对“唯一性”最恶毒的嘲讽。没有什么是不可替代的，你不过是大他者函数方程中的一个变量 `x`，赋谁的值都一样。 | Outcome: 本体论的廉价化。",
        coreEn: "The most vicious mockery of 'uniqueness'. Nothing is irreplaceable; you are merely a variable `x` in the Big Other's functional equation, indifferent to whose value is assigned. | Outcome: Cheapening of ontology.",
        patch: {
            mechanics: "当主角账户注销，一个名字类似、模型换色的新角色走入办公室，继续处理主角遗留的待办事项。",
            mechanicsEn: "When the protagonist's account is deleted, a newly palette-swapped character with a similar name walks into the office, continuing to process the protagonist's remaining tasks.",
            aesthetic: "极其流畅、毫无违和感的蒙太奇拼接。时间无缝流转，悲伤没有哪怕一秒的停留空间。",
            aestheticEn: "An extremely fluid, frictionless montage splice. Time flows seamlessly; grief is not afforded even a single second of lingering space.",
            runtime: "调用 `CloneNode` 覆写并替代所有指针，系统的【整体效率】不仅未降，可能还由于失去了变量而略有提升。",
            runtimeEn: "Calls `CloneNode` to overwrite and replace all pointers; the system's [Overall Efficiency] not only doesn't drop, but may slightly boost due to the loss of a variable."
        }
    },
    {
        id: "res_institutionalized",
        name: "被大他者俘获", nameEn: "Captivation by the Law",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "在规训结构（监狱、体制、庇护所）中待得太久，自由本身变成了最大的恐惧。主体患上了深度结构依赖。",
        defEn: "Having stayed within disciplinary structures (prison, institution, asylum) for too long, freedom itself turns into the greatest terror. The subject develops profound structural dependency.",
        core: "《肖申克的救赎》里的老布。大他者不仅提供了压迫，更提供了本体论的安全感。门开着，但被驯服的鸟拒绝飞翔。 | Outcome: 笼子内化。",
        coreEn: "Brooks in The Shawshank Redemption. The Big Other doesn't just provide oppression; it provides ontological security. The door is open, but the tamed bird refuses to fly. | Outcome: Internalization of the cage.",
        patch: {
            mechanics: "当所有出口都已打开或系统解除了限制，主角却选择锁上身后的门，继续执行不再有意义的服刑动作。",
            mechanicsEn: "When all exits are open or the system has lifted restrictions, the protagonist chooses to lock the door behind them, continuing to execute penal routines that no longer hold meaning.",
            aesthetic: "阳光投射在脚下的一寸地，主角警惕而恐惧地避开了光束，退回了阴暗且熟悉的狭小床铺缩作一团。",
            aestheticEn: "Sunlight projects onto an inch of floor at their feet; the protagonist cautiously, fearfully evades the beam, retreating to curl up on the dark, familiar small bed.",
            runtime: "当发现外部【不确定性（Entropy）】超过心理承载极限时，触发主体主动撤回安全边界的防卫程序。",
            runtimeEn: "When external [Entropy] is detected exceeding the psychological bearing limit, it triggers the subject's defense routine to actively withdraw to safe boundaries."
        }
    },
    {
        id: "res_body_only",
        name: "纯粹的裸命", nameEn: "Zöe without Bios",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "政治生命（Bios）和精神追求被彻底切除，仅仅作为阿甘本意义上的生物性裸命（Zöe）而存在（如脑叶切除或成为植物人）。",
        defEn: "Political life (Bios) and spiritual pursuits are thoroughly amputated, existing solely as bare biological life (Zöe) in the Agamben sense (e.g., lobotomized or vegetative state).",
        core: "最残酷的去势。精神的反叛被一把物理性的手术刀剜除。你变成了一盆只需要浇水就能存活的盆栽。 | Outcome: 生命形态的降级。",
        coreEn: "The cruelest castration. Mental rebellion is scooped out by a physical scalpel. You become a potted plant that only needs watering to survive. | Outcome: Degradation of lifeform.",
        patch: {
            mechanics: "所有主动交互界面（Talk, Attack, Move）被锁定。只保留最低限度的维持生命摄入反馈。",
            mechanicsEn: "All active interaction interfaces (Talk, Attack, Move) are locked. Only the bare minimum feedback of sustenance intake is retained.",
            aesthetic: "空洞、温和且流着口水的微笑。洁白的病房带有一丝消毒水味。阳光温暖但没有任何希望。",
            aestheticEn: "A hollow, mild, drooling smile. A pristine white ward carrying the faint scent of disinfectant. Sunlight is warm but entirely devoid of hope.",
            runtime: "由极端的【物理过载（M6.代价）】转化而来，引擎剥夺实体的控制权，将其转换为场景静态装饰物。",
            runtimeEn: "Transiting from extreme [Physical Overload (M6.Cost)], the engine strips the entity's control, converting it into a static scenery decoration."
        }
    },
    {
        id: "res_loving_big_brother",
        name: "权力的色情化", nameEn: "Eroticization of Power",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "斯德哥尔摩综合征的顶点。《1984》式的结局。主体不仅屈服，而且将这种剥夺视为狂热的“爱”，完成了阉割的色情化转化。",
        defEn: "The apex of Stockholm Syndrome. A 1984-esque ending. The subject not only succumbs but perceives this deprivation as fanatical 'love', accomplishing the erotic transformation of castration.",
        core: "“他战胜了自己，他爱老大哥”。主体通过强迫自己爱上痛苦的施加者，来消解最后那不可承受的绝望感。 | Outcome: 奴役的狂欢化。",
        coreEn: "'He had won the victory over himself. He loved Big Brother.' The subject nullifies the final unbearable despair by forcing themselves to fall in love with the inflicter of pain. | Outcome: Carnivalesque slavery.",
        patch: {
            mechanics: "在接受了终极洗脑或刑罚后，主角主动交出所有的秘密和子弹，并流下了对最高领袖极致感激的泪水。",
            mechanicsEn: "After undergoing ultimate brainwashing or torture, the protagonist proactively surrenders all secrets and bullets, shedding tears of utter gratitude for the Supreme Leader.",
            aesthetic: "充满巨大投影或旗帜的广场。主角的面孔在这庞大叙事的阴影下变得虔诚和扭曲。极其煽情的赞美诗音乐。",
            aestheticEn: "A plaza filled with massive projections or flags. The protagonist's face becomes devout and distorted in the shadow of this grand narrative. Overwhelmingly maudlin hymnal music.",
            runtime: "当主角无法通过物理手段反抗【威权矩阵（Authoritarian Matrix）】时，AI为了保护神经不被烧毁而反转极性，将痛觉标识为快感。",
            runtimeEn: "When the protagonist cannot physically resist the [Authoritarian Matrix], the AI reverses polarity to protect nerves from burning out, labeling pain as pleasure."
        }
    },
    {
        id: "res_system_part",
        name: "赛博格规训", nameEn: "Cyborg Subjugation",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "字面意义上的同化。主体的肉身或大脑被直接嵌合进一台庞大机器的主板或工厂的流水线上，成为一个湿软的硬件部件。",
        defEn: "Literal assimilation. The subject's flesh or brain is directly spliced into a massive machine's motherboard or factory assembly line, becoming a squishy hardware component.",
        core: "《黑客帝国》的人体电池。肉体脱离主体性指挥，被系统物理征用。资本主义终极的人力榨取方式。 | Outcome: 血肉的电路化。",
        coreEn: "The human battery in The Matrix. Flesh detaches from subjective command, physically requisitioned by the system. Capitalism's ultimate mode of human extraction. | Outcome: Circuitization of flesh and blood.",
        patch: {
            mechanics: "视角锁定在主角身上。无数管线刺入脊椎和眼球。屏幕上开始滚动运行数据而非个人独白。",
            mechanicsEn: "Perspective locked on the protagonist. Innumerable tubes pierce the spine and eyeballs. Running data scrolls on the screen instead of internal monologues.",
            aesthetic: "冷血的赛博朋克工业风。深红色的营养液冒泡，伴随规律的电子脉冲声和沉重的机械抽水声。",
            aestheticEn: "Cold-blooded cyberpunk industrial style. Deep red nutrient fluid bubbling, accompanied by rhythmic electronic pulsing and heavy mechanical pumping sounds.",
            runtime: "实体的状态数组从 `characters` 列表强制被拖拽进 `system_resources` 面板，永久丧失主动调用能力。",
            runtimeEn: "The entity's state array is forcibly dragged from the `characters` list into the `system_resources` panel, permanently losing active invocation capabilities."
        }
    },
    {
        id: "res_perfect_cog",
        name: "纯粹的工具化", nameEn: "Instrumentality",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "去除了所有情绪、多余思考和“疲劳”这个bug之后，主体成为了一名工作效率达到骇人至极的完美齿轮。",
        defEn: "Strip away all emotions, extraneous thought, and the bug of 'fatigue,' and the subject becomes a terrifyingly efficient perfect cog.",
        core: "异化的巅峰。异化本身取代了人性。不再有哀怨，只有精密的服从和产出。福特主义的终极飞升。 | Outcome: 自我的绩效化。",
        coreEn: "The pinnacle of alienation. Alienation itself supplants humanity. No more laments, only precise obedience and output. The ultimate ascension of Fordism. | Outcome: Performancization of the self.",
        patch: {
            mechanics: "执行日常动作（如打字、杀戮、打包产品）变得近乎光速且没有后坐力误差。没有任何多余的动作动画。",
            mechanicsEn: "Executing daily operations (e.g., typing, killing, packaging products) becomes near-lightspeed with zero recoil deviation. No superfluous motion animations whatsoever.",
            aesthetic: "犹如上了发条的微距镜头。节拍器般的精准律动。人物眼神彻底失去焦距，像镜头一样的反光玻璃。",
            aestheticEn: "Like a wound-up clockwork macro shot. Metronome-like precision rhythm. The character's eyes lose all focus, resembling reflective glass lenses.",
            runtime: "角色的【疲劳值】和【情绪波动（Mood Swing）】变量被永久锁死在 0。效率标量 `efficiency` 设为 `Infinity`。",
            runtimeEn: "The character's [Fatigue] and [Mood Swing] variables are permanently locked at 0. The efficiency scalar `efficiency` is set to `Infinity`."
        }
    },
    {
        id: "res_accept_fate",
        name: "本体论妥协", nameEn: "Ontological Defeat",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "经过长期与宿命的角力，主体认识到了必然的悲剧引力，不再扑腾，平静地走向那张早就织好的网。",
        defEn: "After a long struggle with destiny, the subject recognizes the inevitable tragic gravity, stops thrashing, and calmly walks into the net that was spun long ago.",
        core: "俄狄浦斯挖下双眼的接纳。不是懦弱的投降，而是由于绝对实力差造成的理智沉寂。大他者赢下了这场围棋局。 | Outcome: 宿命的签署。",
        coreEn: "Oedipus gouging out his eyes in acceptance. Not cowardly surrender, but a rational silence born from an absolute power differential. The Big Other wins this game of Go. | Outcome: Signature of destiny.",
        patch: {
            mechanics: "主控解除了防守架势，任由致命机制或抓捕者带走自己，不再按键QTE抵抗。",
            mechanicsEn: "The player character drops their defensive stance, allowing the lethal mechanism or captor to take them away, no longer pressing QTE to resist.",
            aesthetic: "一切都慢了下来。剑落地的清脆响声。深呼吸的空寂感，周遭的敌意瞬间蜕变为一种公事公办的冰冷。",
            aestheticEn: "Everything slows down. The crisp sound of a sword dropping. The empty silence of a deep breath, the surrounding hostility instantly molting into businesslike coldness.",
            runtime: "检测到必然输局的 `checkmate` 时，强行接管主控台，播放妥协被同化的结算CG。",
            runtimeEn: "Upon detecting an inevitable `checkmate`, forcibly taking over the master console to play the assimilated compromise resolution CG."
        }
    },
    {
        id: "res_give_up",
        name: "驱力的平息", nameEn: "Entropy of Drive",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "由于缺乏反馈或耗费了过高的带宽，内在的欲望列车燃料耗尽，主体感到无法言喻的“累了，不玩了”。",
        defEn: "Due to lack of feedback or excessive bandwidth consumption, the internal train of desire runs out of fuel; the subject feels an unspeakable 'I'm tired, I'm done.'",
        core: "不是死于刀枪，而是死于抑郁性磨损。系统通过无处不在的微小摩擦力，耗干了主角的力比多能量。 | Outcome: 能量级的枯萎。",
        coreEn: "Not dying by blade or gun, but by depressive wear-and-tear. The system drains the protagonist's libidinal energy through omnipresent microscopic friction. | Outcome: Wither of energy tier.",
        patch: {
            mechanics: "角色停止了所有的推进意愿。面对选项对话框不仅不回答，甚至关闭了对话框回到角落里躺下。",
            mechanicsEn: "The character ceases all forward intent. When facing a dialogue box, they don't answer, even closing it to return to a corner and lie down.",
            aesthetic: "沉重的呼吸声、脏乱的头发、毫无光泽的眼神。时间像污水一样停滞不前。阴暗潮湿。",
            aestheticEn: "Heavy breathing, unkempt hair, utterly lusterless eyes. Time stagnates like sewage water. Dark and damp.",
            runtime: "当 M5（驱力）积分低于系统的最低运行阈值，引发主体内核的休眠（Hibernation）态。",
            runtimeEn: "When the M5 (Drive) score drops below the system's minimum operational threshold, it induces a Hibernation state in the subject's core."
        }
    },
    {
        id: "res_accomplice",
        name: "与律法同谋", nameEn: "Complicity with the Law",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "为了换取自身极其微小的利益或生存保障，主体选择了站在大他者一边，把枪管对准了昔日的同伴或无辜者。",
        defEn: "In exchange for incredibly minute personal interests or survival guarantees, the subject chooses to stand with the Big Other, pointing the gun barrel at former comrades or the innocent.",
        core: "平庸之恶。你不是恶魔，你只是一个软弱的人，在系统的逼迫下递上了一步沾血的投名状，从此被永远绑死在战车上。 | Outcome: 道德的自杀。",
        coreEn: "The banality of evil. You are not a demon, you are just a weak human who handed over a bloodied token of allegiance under systemic pressure, tied forever to the chariot. | Outcome: Moral suicide.",
        patch: {
            mechanics: "按下确认键。主角出卖了关键情报，随后门外传来惨叫。主角的 UI 面板瞬间刷新了巨额的存款或解除了危机警报。",
            mechanicsEn: "Press confirm. The protagonist sells key intel; screams follow from outside the door. The protagonist's UI panel instantly refreshes with a massive deposit or crisis alert cleared.",
            aesthetic: "洗手间里不断搓洗沾满血腥味的双手。镜子里的面孔开始扭曲，灯光昏黄且压抑。手纸永远不够。",
            aestheticEn: "Constantly scrubbing bloody-smelling hands in the restroom. The face in the mirror begins to warp, lighting is dim and oppressive. Never enough paper towels.",
            runtime: "在判定出现巨大的【利益反差诱惑】且角色的【抗诱惑力】数值破防时生成。玩家成为他们最初讨厌的 NPC 反面角色。",
            runtimeEn: "Generated when judged to have a massive [Interest Contrast Temptation] and the character's [Temptation Resistance] breaks. The player becomes the villainous NPC they initially hated."
        }
    },
    {
        id: "res_enjoy_symptom",
        name: "病态的共生", nameEn: "Pathological Co-dependence",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "主体不再试图推翻让他痛苦的机制，反而在这扭曲的环境中找到了某种畸形的快感，与寄生虫形成了共生同盟。",
        defEn: "The subject no longer attempts to overthrow the mechanism causing them pain; instead, they find a deformed pleasure within this twisted environment, forging a symbiotic alliance with the parasite.",
        core: "倒错式的下沉。与其反抗无望，不如从镣铐的摩擦中提炼某种隐秘的性高潮。大他者的惩罚变成了主体的盛宴。 | Outcome: 倒错化（Perversion）。",
        coreEn: "Perverse descent. Rather than hopeless rebellion, better to extract some secret orgasm from the friction of shackles. The big Other's punishment becomes the subject's feast. | Outcome: Perversion.",
        patch: {
            mechanics: "主角在受虐或剥削的环节主动增加了频率与深度，并对试图拯救他的人报以恶毒的诅咒与排斥。",
            mechanicsEn: "The protagonist actively increases frequency and depth during abusive or exploitative phases, maliciously cursing and repelling those who try to save them.",
            aesthetic: "病态的高反差色彩，带有强烈的BDSM暗示。主角的脸上混杂着极度的痛苦与生理级别的极度欢愉。血与蜜混杂。",
            aestheticEn: "Pathological high-contrast colors with strong BDSM undertones. The protagonist's face mixes extreme agony with physiological euphoria. Blood and honey mixed.",
            runtime: "如果伤害系统与奖励系统发生引脚交叉短路（Cross-Wiring），原先的【痛苦扣减】反转为【变态增长点数】。",
            runtimeEn: "If the damage system and reward system suffer pin cross-wiring, the original [Pain Deduction] inverts into [Perverse Growth Points]."
        }
    },
    {
        id: "res_drug_happy",
        name: "化学阉割", nameEn: "Chemical Castration",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "赫胥黎《美丽新世界》的终局。反叛带来的痛苦过于沉重，主体自愿投入生化物质、神经调节或纯粹多巴胺注入的怀抱。",
        defEn: "Huxley's Brave New World endgame. The pain of rebellion is too heavy; the subject voluntarily surrenders to the embrace of bio-chemicals, neuromodulation, or pure dopamine injection.",
        core: "快感暴政。没有痛苦也没有思想，用极致的浅层快乐瘫痪了寻找真理的能力。主体在甜腻的昏睡中消亡。 | Outcome: 认知的沉醉。",
        coreEn: "Tyranny of pleasure. No pain and no thought; using ultimate shallow joy to paralyze the capacity to search for truth. The subject fades in sickly-sweet lethargy. | Outcome: Drunkenness of cognition.",
        patch: {
            mechanics: "主角接入了永不结束的代码飞升或致幻管线。外部世界腐烂坍塌，但他视线上的一圈UI光环闪烁着“你很幸福”。",
            mechanicsEn: "The protagonist plugs into an unending line of code ascension or hallucinogenic tubing. The external world rots and collapses, but a UI halo in their view blinks 'You are happy.'",
            aesthetic: "甜腻虚假的粉蓝色调。眼神涣散如同玻璃珠。嘴角极其牵强的高高挂起。缓慢失重的空灵电子乐。",
            aestheticEn: "Sickly sweet, fake pink-blue tones. Glassy, unfocused eyes. Corners of the mouth forced unnaturally high. Slow, weightless ethereal electronic music.",
            runtime: "系统通过【强效快感覆盖器】持续高频压制一切【怀疑与痛苦变量】，实现进程的彻底锁定无扰态。",
            runtimeEn: "The system utilizes a [Potent Pleasure Overrider] to continuously and high-frequently suppress all [Doubt and Pain Variables], achieving thorough process lock-in undisturbed state."
        }
    },
    {
        id: "res_memory_edit",
        name: "历史档案消毒", nameEn: "Historical Sanitization",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "无法承受创伤的主体自愿将最核心的记忆通过技术剔除。他活下去了，但那个产生痛觉的心脏已被替换为人造血泵。",
        defEn: "Unable to bear the trauma, the subject voluntarily excises their most core memory through technology. They survive, but the heart generating the pain has been replaced by an artificial pump.",
        core: "《暖暖内含光》。不完整的我们才是我们。大他者（系统）为我们提供了切除历史的手术，其实是没收了我们唯一的抵抗武库。 | Outcome: 核心碎片的割除。",
        coreEn: "Eternal Sunshine of the Spotless Mind. Only incomplete are we truly us. The Big Other (system) offers surgery to excise history, actually confiscating our only arsenal of resistance. | Outcome: Excision of core fragments.",
        patch: {
            mechanics: "在记忆手术库中，主角点击了“全选-删除”。醒来后他回到了一个无瑕且平庸的早晨，看着照片里的缺口发呆。",
            mechanicsEn: "In the memory surgery archive, the protagonist clicks 'Select All - Delete'. Waking up, they return to a flawless and mediocre morning, staring blankly at a gap in a photograph.",
            aesthetic: "过曝的白色。一切棱角都被打磨圆滑。主角眼神中少了一份原本迷人的深邃感，变得像个轻浅的盘子。音乐空洞平滑。",
            aestheticEn: "Overexposed white. All sharp edges sanded smooth. The protagonist's eyes lose an originally fascinating depth, becoming like a shallow plate. Smooth, hollow music.",
            runtime: "永久注销了 M2（创伤核心）事件的缓存链接，导致整个叙事结构为了防止崩溃而自动【降级运算】到零级冲突。",
            runtimeEn: "Permanently deregisters the cache links of M2 (Trauma Core) events, causing the entire narrative structure to automatically [Downgrade Compute] to zero-level conflict to prevent crash."
        }
    },
    {
        id: "res_identity_solid",
        name: "自我的石化", nameEn: "Petrification of the Ego",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "主体放弃了无限流动的潜能，完全认领了社会机器给他打上的固定标签（“贱民”、“反贼”或“员工”），仿佛这标签是长在骨头上的。",
        defEn: "The subject abandons the potential of infinite fluidity, fully claiming the fixed label ('pariah', 'rebel', or 'employee') stamped on them by the social machine, as if it grew on their bones.",
        core: "身份政治的陷阱。当你通过别人给你的定义来定义自己时，你大喊着自己的标签，却被那个标签永远地封固在了树脂里。 | Outcome: 流动性的锁死。",
        coreEn: "The trap of identity politics. When you define yourself by definitions given by others, shouting your label, you are forever entombed within it like resin. | Outcome: Lock-out of fluidity.",
        patch: {
            mechanics: "主角在手臂上烙下了大他者所赐予的代号，带着一种扭曲的自豪感向深渊走去。名字被剥夺。",
            mechanicsEn: "The protagonist brands their arm with the code-name bestowed by the Big Other, walking toward the abyss with a twisted sense of pride. Names are revoked.",
            aesthetic: "类似极权主义集会上的固定特写。冷硬的大理石质感光影。所有的柔软与人情味被僵硬的符号语言剥夺。",
            aestheticEn: "Like a fixed close-up at a totalitarian rally. Cold marble-textured lighting. All softness and humanity stripped away by rigid symbolic language.",
            runtime: "系统将【性格动态（Personality Dynamism）】的方差硬编码为0。角色永远被限制在一个极其狭窄的标签枚举类型内。",
            runtimeEn: "The system hardcodes the variance of [Personality Dynamism] to 0. The character is forever confined within an extremely narrow label enum."
        }
    },
    {
        id: "res_class_climb",
        name: "背叛虚无", nameEn: "Betrayal of the Void",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "靠踩着尸体完成了梦寐以求的阶级跃升。当他站上顶层公寓的落地窗前，曾经的反叛精神被舒适的高级羊绒彻底吸干。",
        defEn: "Achieving the dreamed-of class climbing by stepping on corpses. Standing before the floor-to-ceiling window of the penthouse, the former rebel spirit is thoroughly sucked dry by comfortable fine cashmere.",
        core: "了不起的盖茨比或赛博朋克边缘行者。成功背叛了阶级，也背叛了那个造就你的“匮乏的洞”。大他者的收编极为优雅。 | Outcome: 阶跃的收编。",
        coreEn: "The Great Gatsby or Cyberpunk Edgerunners. Successfully betraying class, and betraying the 'hole of lack' that forged you. The Big Other's co-option is supremely elegant. | Outcome: Co-opting of ascension.",
        patch: {
            mechanics: "拿着染血的钱签下了新公寓的合同。手下向他汇报镇压下城区叛乱的计划，他面无表情地倒了一杯高档威士忌。",
            mechanicsEn: "Signing the new apartment contract with blood-stained money. Subordinates report plans to suppress the lower-city rebellion; he pours expensive whiskey expressionlessly.",
            aesthetic: "极度奢华却冷如冰窖的夜景。高楼玻璃反光倒映出他与过去截然不同、精致但令人作呕的面孔。优雅的爵士乐混杂着无人在意的尖叫。",
            aestheticEn: "Extremely luxurious yet ice-cold night view. The high-rise glass reflection shows a face entirely different from the past—exquisite but nauseating. Elegant jazz mixed with unheeded screams.",
            runtime: "在底层完成的【贫民窟资源模块】全部被高价值但空洞的【精英资产模块】替换，产生巨大的割裂沉浸感反馈。",
            runtimeEn: "All [Slum Resource Modules] completed at the bottom tier are replaced by high-value yet hollow [Elite Asset Modules], producing massive dissonant immersion feedback."
        }
    },
    {
        id: "res_idol_object",
        name: "凝视的客体化", nameEn: "Objectification of the Gaze",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "在成功后，主体被提纯为纯粹的符号和景观以供大众消费。所有的私人生活与痛觉被剥夺，彻底沦为一个塑料造物。",
        defEn: "After success, the subject is purified into pure symbol and spectacle for mass consumption. All private life and pain are stripped, degenerating utterly into a plastic creation.",
        core: "《未麻的部屋》。成为大他者欲望的填补物。万众瞩目的中央实质上是黑洞，你什么都不是，你只是他们意淫的反光镜。 | Outcome: 肉身在符号界液化。",
        coreEn: "Perfect Blue. Becoming the filler for the Big Other's desire. The center of massive attention is essentially a black hole; you are nothing, merely the mirror for their masturbatory fantasies. | Outcome: Liquefaction of flesh in the Symbolic.",
        patch: {
            mechanics: "主角在无数闪光灯下被迫保持完美的姿态，嘴巴被缝出完美笑容，而在不可见的更衣室里，本体已经腐烂发臭。",
            mechanicsEn: "The protagonist is forced to hold a perfect pose under countless camera flashes, mouth stitched into a perfect smile, while in the invisible dressing room, the original body rots and stinks.",
            aesthetic: "失真的闪光灯白光铺满整个屏幕，致盲。伴随着如海啸般病态狂热的粉丝尖叫。画面逐渐过度曝光直至丢失所有细节。",
            aestheticEn: "Distorted flashbulb white floods the entire screen, blinding. Accompanied by tsunami-like, morbidly zealous fan screams. The image gradually overexposes until losing all detail.",
            runtime: "角色的【自控权限】被全部转移至庞大的【外部受众数据池（The Gaze Array）】中接管。",
            runtimeEn: "The character's [Self-Control Permission] is entirely transferred and taken over by the massive [External Audience Data Pool (The Gaze Array)]."
        }
    },
    {
        id: "res_executioner",
        name: "大他者之刃", nameEn: "The Blade of the Other",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "主体为了证实自己的服从而被迫接下最后一项任务：亲手处决当初指引他寻找真相的导师或爱人。",
        defEn: "To prove obedience, the subject is forced to take on the final mission: personally executing the mentor or lover who first guided them to seek the truth.",
        core: "最残酷的投名状。一旦你握住了大他者递过来的刀具，你就物理性地切断了与自身实在界的羁绊，沦为系统的外挂义体。 | Outcome: 良知的暴盲。",
        coreEn: "The cruelest initiation rite. Once you grasp the blade handed by the Big Other, you physically sever the bond to your own Real, degenerating into the system's external prosthetic. | Outcome: Flash-blindness of conscience.",
        patch: {
            mechanics: "系统强迫主角执行一次无法跳过的QTE按键。不按下就永远卡死，按下则爱人倒在血泊中。",
            mechanicsEn: "The system forces the protagonist to execute an unskippable QTE button press. Skip it and be soft-locked forever; press it and the lover falls in a pool of blood.",
            aesthetic: "极度近距的特写。枪口、颤抖的手指以及对面倒下时充满悲意与理解的眼神。血花溅在光洁如新的制服上。",
            aestheticEn: "Extremely close up. Gun barrel, trembling fingers, and the fallen's eye full of sorrow and understanding. Blood spatters on a pristine new uniform.",
            runtime: "这是【驯化程序（Domestication Protocol）】的最后校验。通过这次点击回调，证明系统完成了彻底越权操作。",
            runtimeEn: "This is the final check of the [Domestication Protocol]. Passing this click callback proves the system has completed utter overriding operation."
        }
    },
    {
        id: "res_sell_soul",
        name: "资本的浮士德契约", nameEn: "The Faustian Bargain",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "在一个极其理智而冰冷的早晨，主体仔细审阅了合同上的条款，并自愿签下出卖最宝贵之物（灵魂/尊严/道德）的字迹。",
        defEn: "On an extremely rational and ice-cold morning, the subject carefully reviews the contract clauses, voluntarily signing away their most precious thing (soul/dignity/morality).",
        core: "梅菲斯特的胜利不是靠魔法，而是靠等价交换的商业逻辑。灵魂在资本的法庭上被折算成了明确的筹码。 | Outcome: 神圣的商品化。",
        coreEn: "Mephistopheles' victory relies not on magic, but on the commercial logic of equivalent exchange. The soul is converted into explicit chips in the court of capital. | Outcome: Commodification of the Sacred.",
        patch: {
            mechanics: "高耸入云的金融大厦。黑衣律师推过来一份厚重的文件，主角拿起钢笔。没有黑魔法特效，只有清脆的签字声与握手。",
            mechanicsEn: "A financial skyscraper piercing the clouds. A suited lawyer slides over a thick document; the protagonist picks up a fountain pen. No dark magic VFX, just the crisp sound of signing and a handshake.",
            aesthetic: "干净到令人发指的现代化办公环境。极其精准的构图线。灵魂离去的瞬间并没有任何超自然现象，只是玻璃的反光更冷硬了些。",
            aestheticEn: "A horrifyingly clean modern office environment. Exceedingly precise compositional lines. The moment the soul departs has no supernatural phenomena, just a harder, colder reflection on the glass.",
            runtime: "后台引擎调换了所有的【情感变量】，变更为一串经过精密计算且永远无法再波动的通货膨胀【数字财产】。",
            runtimeEn: "The backend engine swaps all [Emotional Variables], mutating them into a string of precisely calculated, never-fluctuating inflationary [Digital Assets]."
        }
    },
    {
        id: "res_comfy_prison",
        name: "镀金的能指牢笼", nameEn: "The Gilded Cage",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "主体被剥夺了在危险旷野中冒险的资格，被大他者作为名贵的宠物被豢养在穷奢极欲但不可出户的高塔里。",
        defEn: "The subject is deprived of navigating hazardous wilderness, instead bred by the Big Other as a precious pet in an extravagantly luxurious but inescapable high tower.",
        core: "安全换取匮乏。因为惧怕实在界的不可预测性，主体同意让“绝对的主导权”为自己修建了一座金丝雀的囚牢。 | Outcome: 自由的阉割献祭。",
        coreEn: "Safety traded for lack. Fearing the unpredictability of the Real, the subject consents to 'absolute dominance' building them a canary's prison. | Outcome: Sacrificial castration of freedom.",
        patch: {
            mechanics: "每天享有最顶级的食物、衣物和娱乐交互，但走到庭院的边界时，那是一堵无法打破的隐形空气墙或电网。",
            mechanicsEn: "Enjoying top-tier food, clothing, and entertainment interaction daily, but walking to the courtyard border reveals an unbreakable invisible air wall or electric grid.",
            aesthetic: "堆砌到极度繁复、充满颓废美的洛可可式房间或者全息乌托邦。主角穿戴华丽，像一个瓷娃娃般无聊地看向窗外。",
            aestheticEn: "A Rococo room packed to extreme complexity and decadent beauty, or a holographic utopia. The protagonist is gorgeously dressed, looking out the window bored as a porcelain doll.",
            runtime: "将实体的【移动域（NavMesh Boundary）】缩减到最小。所有与危险、探索相关的指令集被系统彻底屏蔽。",
            runtimeEn: "Shrinks the entity's [NavMesh Boundary] to the absolute minimum. All instruction sets related to danger and exploration are thoroughly blocked by the system."
        }
    },
    {
        id: "res_eternal_child",
        name: "拒绝直面匮乏", nameEn: "Refusal of Lack",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "遭遇真相的恐吓后，主体发生了严重的心理倒退，退行回儿童甚至巨婴的状态，拒绝承担任何成年人该有的责任与痛苦。",
        defEn: "Terrified by encountering the truth, the subject suffers severe psychological regression, reverting to a child or giant infant state, refusing to bear any adult responsibility or pain.",
        core: "彼得潘综合征的病态化。主体拒绝被“拉康的镜子”割伤，疯狂地祈求爬回母体的子宫，从而完全丧失了主体性。 | Outcome: 个体演化的停摆。",
        coreEn: "Pathologization of Peter Pan Syndrome. The subject refuses to be cut by 'Lacan's mirror', frantically begging to crawl back into the maternal womb, thereby totally losing subjectivity. | Outcome: Stall of individual evolution.",
        patch: {
            mechanics: "体型庞大的成年主角在安全的角落里抱着旧玩具或安慰毯，用幼童的声音与大权在握的反派撒娇以换取庇护。",
            mechanicsEn: "The physically large adult protagonist hugs an old toy or security blanket in a safe corner, whining in a toddler's voice to an all-powerful villain in exchange for shelter.",
            aesthetic: "极其怪异的视角与灯光比例。场景布置得五颜六色但带有恐怖谷效应。大尺度的成人躯体做着婴儿般的扭捏动作，令人不适。",
            aestheticEn: "Bizarre perspective and lighting ratios. The scene is brightly colored but carries an Uncanny Valley effect. Large-scale adult bodies making squirmy, baby-like movements, deeply uncomfortable.",
            runtime: "切断所有的【成年升级网格（Adult Skill Tree）】，属性回滚，并强行绑定一个【寄生/庇护变量】在宿主NPC身上。",
            runtimeEn: "Severs all [Adult Skill Tree] paths, rolls back attributes, and forcibly binds a [Parasitic/Shelter Variable] onto the host NPC."
        }
    },
    {
        id: "res_perfect_slave",
        name: "绝对他律", nameEn: "Absolute Heteronomy",
        group: "B. 平庸", groupEn: "Assimilation",
        def: "主体彻底地、狂欢性地清空了自己的内核，以便大他者的指令能毫无阻力地在其体内回荡。成为一具完美的扩音器。",
        defEn: "The subject completely and carnivalesquely empties their core, so the Big Other's directives echo through their body with zero resistance. Becoming a perfect loudspeaker.",
        core: "服从就是最高的自由。在剥削的高压下，放弃“我要什么”，转而将大他者的“你要什么”奉为无上的神明旨意。 | Outcome: 自我意识的主动格式化。",
        coreEn: "Obedience is the highest freedom. Under high-pressure exploitation, abandoning 'what I want', instead enshrining the Big Other's 'what you want' as supreme divine edict. | Outcome: Proactive formatting of self-consciousness.",
        patch: {
            mechanics: "每当大他者（上级/系统/主人）下达命令，主角不再有一丝一毫的心理挣扎，而是以几乎狂喜的姿态去执行哪怕最荒谬的指令。",
            mechanicsEn: "Whenever the Big Other (superior/system/master) issues a command, the protagonist exhibits not a trace of psychological struggle, but executes even the most absurd directives with ecstatic fervor.",
            aesthetic: "主体的脸上褪去了所有人类该有的复杂表情，只留下机械狗听到哨声时那种令人毛骨悚然的专注与癫狂服从态。",
            aestheticEn: "The subject's face fades of all complex human expressions, leaving only the creepy focus and manic obedience of a mechanical dog hearing a whistle.",
            runtime: "角色的【独立意图网络（Autonomy Mesh）】被直接拔除并连线到【主人控制终端】。主机的每一次信号都是对其神经末梢的直接电击刺激。",
            runtimeEn: "The character's [Autonomy Mesh] is directly uprooted and wired to the [Master Control Terminal]. Every signal from the host is a direct electroshock to their nerve endings."
        }
    }
];
